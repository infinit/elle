...
namespace network_boost { namespace proto
{
    ...
    ///
    /// <tt>network_boost::result_of\<make\<T\<X0,X1,...\> \>(Expr, State, Data)\>::type</tt>
    /// is evaluated as follows. For each \c X in <tt>X0,X1,...</tt>, do:
    ///
    /// \li If \c X is a template like <tt>U\<Y0,Y1,...\></tt>, then let <tt>X'</tt>
    ///     be <tt>network_boost::result_of\<make\<U\<Y0,Y1,...\> \>(Expr, State, Data)\>::type</tt>
    ...
    ///     <tt>network_boost::result_of\<when\<_, X\>(Expr, State, Data)\>::type</tt>.
