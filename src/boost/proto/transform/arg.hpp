...
namespace network_boost { namespace proto
{
    ...
    /// assert( network_boost::addressof(i) == network_boost::addressof(j) );
    /// \endcode
    ...
    /// in a \c network_boost::reference_wrapper\<\>.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// network_boost::reference_wrapper<proto::terminal<int>::type> j
    ///     = proto::when<_, proto::_byref(_)>()(i);
    /// assert( network_boost::addressof(i) == network_boost::addressof(j.get()) );
    /// \endcode
    struct _byref : callable
    {
        ...
        {
            typedef network_boost::reference_wrapper<T const> const type;
        };

        template<typename This, typename T>
        struct result<This(T &)>
        {
            typedef network_boost::reference_wrapper<T> const type;
        };

        /// Wrap the parameter \c t in a \c network_boost::reference_wrapper\<\>
        /// \param t The object to wrap
        /// \return <tt>network_boost::ref(t)</tt>
        /// \throw nothrow
        template<typename T>
        network_boost::reference_wrapper<T> const operator ()(T &t) const
        {
            return network_boost::reference_wrapper<T>(t);
        }

        /// \overload
        ///
        template<typename T>
        network_boost::reference_wrapper<T const> const operator ()(T const &t) const
        {
            return network_boost::reference_wrapper<T const>(t);
        }
    };

    /// \brief A unary CallableTransform that strips references
    /// and \c network_boost::reference_wrapper\<\> from its argument.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// int j = 67;
    /// int k = proto::when<_, proto::_byval(proto::_state)>()(i, network_boost::ref(j));
    /// assert( 67 == k );
    /// \endcode
    struct _byval : callable
    {
        ...
        template<typename This, typename T>
        struct result<This(network_boost::reference_wrapper<T>)>
          : result<This(T)>
        {};
        ...
        ///
        template<typename T>
        T operator ()(network_boost::reference_wrapper<T> const &t) const
        {
            return t;
        }
    };
