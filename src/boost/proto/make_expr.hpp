...
#define BOOST_PROTO_AS_CHILD_TYPE(Z, N, DATA)                                                   \
    typename network_boost::proto::detail::protoify<                                                    \
...
#define BOOST_PROTO_AS_CHILD(Z, N, DATA)                                                        \
    network_boost::proto::detail::protoify<                                                             \
        BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(3, 0, DATA), N)                                        \
      , BOOST_PP_TUPLE_ELEM(3, 2, DATA)                                                         \
    ...
    namespace detail
    {
        ...
        template<typename T, typename Domain>
        struct protoify<network_boost::reference_wrapper<T>, Domain>
          : Domain::template as_child<T>
        {};

        template<typename T, typename Domain>
        struct protoify<network_boost::reference_wrapper<T> const, Domain>
          : Domain::template as_child<T>
        {};
    ...
    namespace result_of
    {
        ...
        >
        {
            ...
            /// typedef for <tt>network_boost::result_of\<Domain(expr\<tag::terminal,
            /// term\<A0\> \>)\>::type</tt>.
            ///
            /// Otherwise, \c type is a typedef for <tt>network_boost::result_of\<Domain(expr\<Tag,
            ...
            /// child type is \c A.
            /// \li If \c A is <tt>B &</tt> or <tt>cv network_boost::reference_wrapper\<B\></tt>,
            /// and <tt>is_expr\<B\>::value</tt> is \c true, then the
            /// child type is <tt>B &</tt>.
            /// \li If <tt>is_expr\<A\>::value</tt> is \c false, then the
            /// child type is <tt>network_boost::result_of\<Domain(expr\<tag::terminal, term\<A\> \>
            /// )\>::type</tt>.
            /// \li If \c A is <tt>B &</tt> or <tt>cv network_boost::reference_wrapper\<B\></tt>,
            /// and <tt>is_expr\<B\>::value</tt> is \c false, then the
            /// child type is <tt>network_boost::result_of\<Domain(expr\<tag::terminal, term\<B &\> \>
            /// )\>::type</tt>.
            typedef
                typename detail::make_expr_<
                    Tag
    ...
    /// \li If \c x is a <tt>network_boost::reference_wrapper\<\></tt>,
