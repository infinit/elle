...
# endif
    ...
    path(Source const& source,
      typename network_boost::enable_if<path_traits::is_pathable<
        typename network_boost::decay<Source>::type> >::type* =0)
    {
...
# endif
    ...
    template <class Source>
      typename network_boost::enable_if<path_traits::is_pathable<
        typename network_boost::decay<Source>::type>, path&>::type
    ...
    template <class CharT>
      typename network_boost::enable_if<is_integral<CharT>, path&>::type
    operator+=(CharT c)
    {
      CharT tmp[2];
      tmp[0] = c;
...
# endif
  ...
  private:
    ...
    friend class network_boost::filesystem::path;
    friend class network_boost::filesystem::path::reverse_iterator;
...
# endif
  ...
  //    use network_boost::io::quoted() to handle spaces in paths
  ...
  {
    return os
      << network_boost::io::quoted(p.template string<std::basic_string<Char> >(), static_cast<Char>('&'));
  }
  ...
  operator>>(std::basic_istream<Char, Traits>& is, path& p)
  {
    ...
    is >> network_boost::io::quoted(str, static_cast<Char>('&'));
    p = str;
    return is;
  }
