...
namespace network_boost
{
  namespace filesystem
  {
    ...
    private:
      ...
      network_boost::shared_ptr<m_imp> m_imp_ptr;
    };
...
# endif
  ...
  struct space_info
  {
    ...
    network_boost::uintmax_t capacity;
    network_boost::uintmax_t free;      // <= capacity
    network_boost::uintmax_t available; // <= free
  };
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    ...
    BOOST_FILESYSTEM_DECL
    network_boost::uintmax_t file_size(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
    network_boost::uintmax_t hard_link_count(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
    ...
    BOOST_FILESYSTEM_DECL
    network_boost::uintmax_t remove_all(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
...
# endif
  ...
  inline
  network_boost::uintmax_t file_size(const path& p) {return detail::file_size(p);}

  inline
  network_boost::uintmax_t file_size(const path& p, system::error_code& ec) BOOST_NOEXCEPT
                                       {return detail::file_size(p, &ec);}
  inline
  network_boost::uintmax_t hard_link_count(const path& p) {return detail::hard_link_count(p);}

  inline
  network_boost::uintmax_t hard_link_count(const path& p, system::error_code& ec) BOOST_NOEXCEPT
  ...
  inline
  network_boost::uintmax_t remove_all(const path& p) {return detail::remove_all(p);}
    
  inline
  network_boost::uintmax_t remove_all(const path& p, system::error_code& ec) BOOST_NOEXCEPT
...
{
public:
  typedef network_boost::filesystem::path::value_type value_type;   // enables class path ctor taking directory_entry

  directory_entry() BOOST_NOEXCEPT {}
  explicit directory_entry(const network_boost::filesystem::path& p)
    : m_path(p), m_status(file_status()), m_symlink_status(file_status())
    {}
  directory_entry(const network_boost::filesystem::path& p,
  ...
  void assign(const network_boost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
    { m_path = p; m_status = st; m_symlink_status = symlink_st; }

  void replace_filename(const network_boost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
  {
...
# ifndef BOOST_FILESYSTEM_NO_DEPRECATED
  void replace_leaf(const network_boost::filesystem::path& p,
    file_status st, file_status symlink_st)
      { replace_filename(p, st, symlink_st); }
# endif

  const network_boost::filesystem::path&  path() const BOOST_NOEXCEPT {return m_path;}
  operator const network_boost::filesystem::path&() const BOOST_NOEXCEPT
                                                              {return m_path;}
...
private:
  network_boost::filesystem::path   m_path;
...
//--------------------------------------------------------------------------------------//
  ...
  class directory_iterator
    : public network_boost::iterator_facade< directory_iterator,
                                     directory_entry,
                                     network_boost::single_pass_traversal_tag >
  {
  public:
  ...
  private:
    ...
    network_boost::shared_ptr< detail::dir_itr_imp >  m_imp;

    friend class network_boost::iterator_core_access;

    network_boost::iterator_facade<
      directory_iterator,
      directory_entry,
      network_boost::single_pass_traversal_tag >::reference dereference() const 
    {
  ...
  //  namespace network_boost template specializations
  template<>
  struct range_mutable_iterator<network_boost::filesystem::directory_iterator>
    { typedef network_boost::filesystem::directory_iterator type; };
  template<>
  struct range_const_iterator <network_boost::filesystem::directory_iterator>
    { typedef network_boost::filesystem::directory_iterator type; };
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    ...
    //  clients of struct 'network_boost::filesystem::detail::recur_dir_itr_imp'
...
//--------------------------------------------------------------------------------------//
  ...
  class recursive_directory_iterator
    : public network_boost::iterator_facade<
        recursive_directory_iterator,
        directory_entry,
        network_boost::single_pass_traversal_tag >
  {
...
#   endif
  ...
  private:
    ...
    // shared_ptr provides shallow-copy semantics required for InputIterators.
    // m_imp.get()==0 indicates the end iterator.
    network_boost::shared_ptr< detail::recur_dir_itr_imp >  m_imp;
    ...
    network_boost::iterator_facade< 
      recursive_directory_iterator,
      directory_entry,
      network_boost::single_pass_traversal_tag >::reference
    dereference() const 
    {
      BOOST_ASSERT_MSG(m_imp.get(),
  ...
  //  namespace network_boost template specializations
  template<>
  struct range_mutable_iterator<network_boost::filesystem::recursive_directory_iterator>
                        { typedef network_boost::filesystem::recursive_directory_iterator type; };
  template<>
  struct range_const_iterator <network_boost::filesystem::recursive_directory_iterator>
                        { typedef network_boost::filesystem::recursive_directory_iterator type; };
