// ----------------------------------------------------------------------------
// free_funcs.hpp :  implementation of the free functions of network_boost::format
// ----------------------------------------------------------------------------
...
namespace network_boost {
    ...
    namespace io {
         using ::network_boost::str; // keep compatibility with when it was defined in this N.S.
    }   // - namespace io
    ...
    {
        typedef network_boost::basic_format<Ch, Tr, Alloc>   format_t;
        if(f.items_.size()==0) 
            os << f.prefix_;
        else {
            if(f.cur_arg_ < f.num_args_)
                if( f.exceptions() & io::too_few_args_bit )
                    ...
                    network_boost::throw_exception(io::too_few_args(f.cur_arg_, f.num_args_)); 
