...
namespace detail {
  ...
  class protect_conversion {
      typedef typename network_boost::remove_reference<T>::type non_ref_T;
    public:

  // add const to rvalues, so that all rvalues are stored as const in 
  // the args tuple
    typedef typename detail::IF_type<
      network_boost::is_reference<T>::value && !network_boost::is_const<non_ref_T>::value,
...
// experimental feature
  ...
  typedef typename network_boost::remove_reference<A>::type non_ref_A;
  typedef typename network_boost::remove_reference<B>::type non_ref_B;
