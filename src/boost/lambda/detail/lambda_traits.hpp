...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename network_boost::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    network_boost::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<class T> struct remove_reference_and_cv {
   typedef typename network_boost::remove_cv<
     typename network_boost::remove_reference<T>::type
   >::type type;
};
...
template<int N, class T> struct tuple_element_as_reference {   
  typedef typename
     network_boost::tuples::access_traits<
       typename network_boost::tuples::element<N, T>::type
     >::non_const_type type;
};
...
struct parameter_traits_<network_boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const network_boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<volatile network_boost::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const volatile network_boost::reference_wrapper<T>, Any >{
  typedef T& type;
};
...
struct const_copy_argument {
  typedef typename 
    detail::parameter_traits_<
      T,
      typename detail::IF<network_boost::is_function<T>::value, T&, const T>::RET
    >::type type;
};
...
class bind_type_generator {
  ...
  BOOST_STATIC_CONSTANT(int, nof_elems = network_boost::tuples::length<args_t>::value);
...
} // end of namespace network_boost
