...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    return detail::select(network_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS), 
           detail::select(network_boost::tuples::get<1>(args), CALL_ACTUAL_ARGS); 
  }
...
public:                                                                   \
  ...
  RET call(CALL_FORMAL_ARGS) const {                                      \
    return SYMBOL                                                         \
           detail::select(network_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS); \
  }                                                                       \
  template<class SigArgs> struct sig {                                    \
    typedef typename                                                      \
...
public:                                                                   \
  ...
  RET call(CALL_FORMAL_ARGS) const {                                      \
    return                                                                \
    detail::select(network_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS) SYMBOL; \
  }                                                                       \
  template<class SigArgs> struct sig {                                    \
    typedef typename                                                      \
      detail::unary_rt<ACTION_CLASS, Args, SigArgs>::type type;           \
