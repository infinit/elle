...
template<> struct placeholder<FIRST> {
  ...
  RET call(CALL_FORMAL_ARGS) const { 
    BOOST_STATIC_ASSERT(network_boost::is_reference<RET>::value); 
...
public:
  ...
  // Support for network_boost::result_of.
  ...
  const lambda_functor<lambda_functor_base<
                  ...
                  network_boost::tuple<lambda_functor,
...
template<class T>
struct result_of<network_boost::lambda::lambda_functor<T>()>
{
    typedef typename network_boost::lambda::lambda_functor<T>::nullary_return_type type;
};

template<class T>
struct result_of<const network_boost::lambda::lambda_functor<T>()>
{
    typedef typename network_boost::lambda::lambda_functor<T>::nullary_return_type type;
};
...
template<class T>
struct tr1_result_of<network_boost::lambda::lambda_functor<T>()>
{
    typedef typename network_boost::lambda::lambda_functor<T>::nullary_return_type type;
};

template<class T>
struct tr1_result_of<const network_boost::lambda::lambda_functor<T>()>
{
    typedef typename network_boost::lambda::lambda_functor<T>::nullary_return_type type;
};
