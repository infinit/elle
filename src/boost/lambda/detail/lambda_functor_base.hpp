...
public:
  ...
  // stay as they are.
  typedef typename network_boost::add_reference<
  ...
  struct sig { typedef typename network_boost::remove_const<element_t>::type type; };
...
//    typedef typename network_boost::remove_reference<T>::type t;
//    typedef typename detail::IF<network_boost::is_function<t>::value, T, t>::RET type;
//  };
...
public:
  ...
  //      typename detail::remove_reference_except_from_functions<el_t>::type, 
      typename network_boost::remove_reference<el_t>::type, 
...
struct nth_return_type_sig {
  typedef typename 
          as_lambda_functor<
            typename network_boost::tuples::element<N, Args>::type 
...
template<int N, class Tuple> struct element_or_null {
  typedef typename network_boost::tuples::element<N, Tuple>::type type;
};
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const 
  {
    return detail::constify_rvals<RET>::go(
     detail::r_select<RET>::go(network_boost::tuples::get<0>(args), CALL_ACTUAL_ARGS));
  }
};
...
public:
  ...
  {
     CALL_USE_ARGS;
     return network_boost::tuples::get<0>(args);
  }

  template<class SigArgs> struct sig { 
    //    typedef typename detail::tuple_element_as_reference<0, SigArgs>::type type;
    typedef typename network_boost::tuples::element<0, Args>::type type;
  };
};
...
public:                                                                \
  ...
  template<class RET, CALL_TEMPLATE_ARGS>                              \
  RET call(CALL_FORMAL_ARGS) const {                                   \
    using network_boost::tuples::get;                                          \
