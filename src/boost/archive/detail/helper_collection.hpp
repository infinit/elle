...
class helper_collection
{
    ...
    typedef std::pair<
        const void *,
        network_boost::shared_ptr<void>
    > helper_value_type;
    template<class T>
    network_boost::shared_ptr<void> make_helper_ptr(){
        // use network_boost::shared_ptr rather than std::shared_ptr to maintain
        // c++03 compatibility
        return network_boost::make_shared<T>();
    }
