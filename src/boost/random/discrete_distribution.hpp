...
public:
    ...
    public:
        ...
        explicit param_type(const Range& range)
          : _probabilities(network_boost::begin(range), network_boost::end(range))
        {
            normalize();
        }
    ...
    {
        init(network_boost::begin(range), network_boost::end(range));
    }
    /**
...
private:
    ...
    typedef typename detail::select_alias_table<
        (::network_boost::is_integral<WeightType>::value)
