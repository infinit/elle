...
public:
    ...
    { return max_imp(network_boost::is_integral<UIntType>()); }
    ...
    {
        for(network_boost::uintmax_t i = 0; i < z; ++i) {
            (*this)();
        }
    }
...
private:
    ...
    /// \cond show_private
    typedef typename network_boost::random::traits::make_unsigned<base_result_type>::type base_unsigned;

    static UIntType max_imp(const network_boost::true_type&)
    {
       return network_boost::low_bits_mask_t<w>::sig_bits;
    }
    static UIntType max_imp(const network_boost::false_type&)
    {
