...
public:
    ...
    result_type operator()(Engine& eng) const
    {
        ...
        return generate(eng, network_boost::random::traits::is_integral<base_result>());
    }
...
private:
    ...
    // \cond show_private
    template<class Engine>
    result_type generate(Engine& eng, network_boost::mpl::true_) const
    {
        // equivalent to (eng() - eng.min()) % (_max - _min + 1) + _min,
        // but guarantees no overflow.
        typedef typename Engine::result_type base_result;
        typedef typename network_boost::random::traits::make_unsigned<base_result>::type base_unsigned;
        typedef typename network_boost::random::traits::make_unsigned_or_unbounded<result_type>::type range_type;
#ifdef BOOST_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS
        ...
        if(range >= base_range) {
            return network_boost::random::detail::add<range_type, result_type>()(
    ...
    template<class Engine>
    result_type generate(Engine& eng, network_boost::mpl::false_) const
    {
        typedef typename Engine::result_type base_result;
        typedef typename network_boost::random::traits::make_unsigned<result_type>::type range_type;
        range_type range = random::detail::subtract<result_type>()(_max, _min);
        base_result val = network_boost::uniform_01<base_result>()(eng);
        ...
        return network_boost::random::detail::add<range_type, result_type>()(offset , _min);
    }
    // \endcond
