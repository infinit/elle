...
public:
    ...
    void discard(network_boost::uintmax_t z)
    {
        ...
        if(b_gcd == 1) {
            ...
            network_boost::uintmax_t exponent = z;
            while(exponent != 0) {
                if(exponent % 2 == 1) {
                    a_zm1_over_gcd =
                        mod_type::mult_add(
...
{
public:
    typedef network_boost::uint32_t result_type;
    ...
    /** Advances the state of the generator by @c z. */
    void discard(network_boost::uintmax_t z) { lcf.discard(z); }
...
private:
    ...
    static network_boost::uint64_t cnv(network_boost::uint32_t x)
