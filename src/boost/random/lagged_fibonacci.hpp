...
public:
    ...
    BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(lagged_fibonacci_engine,
        UIntType, value)
    {
        minstd_rand0 intgen(static_cast<network_boost::uint32_t>(value));
    ...
    void discard(network_boost::uintmax_t z)
    {
        for(network_boost::uintmax_t j = 0; j < z; ++j) {
            (*this)();
        }
    }
...
public:
    ...
    BOOST_STATIC_CONSTANT(network_boost::uint32_t, default_seed = 331u);
     ...
     */
    BOOST_RANDOM_DETAIL_ARITHMETIC_SEED(lagged_fibonacci_01_engine, network_boost::uint32_t, value)
    {
...
template<class RealType, int w, unsigned int p, unsigned int q>
const network_boost::uint32_t lagged_fibonacci_01_engine<RealType,w,p,q>::default_seed;
#endif
...
public:
    lagged_fibonacci_01() {}
    BOOST_RANDOM_DETAIL_ARITHMETIC_CONSTRUCTOR(lagged_fibonacci_01, network_boost::uint32_t, val)
    { this->seed(val); }
...
/** @copydoc network_boost::random::detail::lagged_fibonacci_doc */
