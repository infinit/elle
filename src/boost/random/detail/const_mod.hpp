...
public:
  ...
  static IntType pow(IntType a, network_boost::uintmax_t exponent)
  {
      IntType result = 1;
      while(exponent != 0) {
          if(exponent % 2 == 1) {
...
private:
  ...
  static IntType mult_general(IntType a, IntType b)
  {
    ...
    if(::network_boost::uintmax_t(modulus) <=
        (::std::numeric_limits< ::network_boost::uintmax_t>::max)() / modulus)
    {
      return static_cast<IntType>(network_boost::uintmax_t(a) * b % modulus);
    } else {
