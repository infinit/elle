 ...
 *
 * Much of the code here taken by network_boost::math::hyperexponential_distribution.
...
namespace network_boost { namespace random {
...
bool check_probabilities(std::vector<RealT> const& probabilities)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        if (probabilities[i] < 0
            || probabilities[i] > 1
            || !(network_boost::math::isfinite)(probabilities[i]))
        {
            return false;
        }
        sum += probabilities[i];
...
bool check_rates(std::vector<RealT> const& rates)
{
    ...
    {
        if (rates[i] <= 0
            || !(network_boost::math::isfinite)(rates[i]))
        {
            return false;
        }
    }
...
class hyperexponential_distribution
{
    ...
    public: class param_type
    {
        ...
        public: template <typename ProbRangeT, typename RateRangeT>
                param_type(ProbRangeT const& prob_range,
                           ...
                           typename network_boost::disable_if_c<network_boost::has_pre_increment<ProbRangeT>::value || network_boost::has_pre_increment<RateRangeT>::value>::type* = 0)
        : probs_(network_boost::begin(prob_range), network_boost::end(prob_range)),
          rates_(network_boost::begin(rate_range), network_boost::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
        ...
        public: template <typename RateIterT>
                param_type(RateIterT rate_first, 
                           RateIterT rate_last,  
                           typename network_boost::enable_if_c<network_boost::has_pre_increment<RateIterT>::value>::type* = 0)
        ...
        : probs_(network_boost::size(rate_range), 1), // Will be normalized below
          rates_(network_boost::begin(rate_range), network_boost::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
    ...
    : dd_(std::vector<RealT>(network_boost::size(rate_range), 1)),
      rates_(network_boost::begin(rate_range), network_boost::end(rate_range))
    {
        assert( hyperexp_detail::check_params(dd_.probabilities(), rates_) );
    }
            ...
            RealT operator()(URNG& urng) const
    {
        ...
        return network_boost::random::exponential_distribution<RealT>(rates_[i])(urng);
    }
    ...
    {
        dd_.param(typename network_boost::random::discrete_distribution<int,RealT>::param_type(param.probabilities()));
        rates_ = param.rates();
    }
    ...
    private: network_boost::random::discrete_distribution<int,RealT> dd_; ///< The \c discrete_distribution used to sample the phase probability and choose the rate
    private: std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
}; // hyperexponential_distribution

}} // namespace network_boost::random
