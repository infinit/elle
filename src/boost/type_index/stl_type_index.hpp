...
/// \brief Contains network_boost::typeindex::stl_type_index class.
///
/// network_boost::typeindex::stl_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
/// is defined network_boost::typeindex::ctti is usually used instead of network_boost::typeindex::stl_type_index.
...
inline std::string stl_type_index::pretty_name() const {
    static const char cvr_saver_name[] = "network_boost::typeindex::detail::cvr_saver<";
    ...
    const network_boost::core::scoped_demangled_name demangled_name(data_->name());

    const char* begin = demangled_name.get();
    if (!begin) {
        network_boost::throw_exception(std::runtime_error("Type name demangling failed"));
    }
...
#else
    return network_boost::hash_range(raw_name(), raw_name() + std::strlen(raw_name()));
#endif
}
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME network_boost::remove_reference<T>::type no_ref_t;
    typedef BOOST_DEDUCED_TYPENAME network_boost::remove_cv<no_ref_t>::type no_cvr_prefinal_t;
    ...
    #  if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) \
        ...
        typedef BOOST_DEDUCED_TYPENAME network_boost::mpl::if_<
            network_boost::is_signed<no_cvr_prefinal_t>,
            network_boost::make_signed<no_cvr_prefinal_t>,
            network_boost::mpl::identity<no_cvr_prefinal_t>
...
inline stl_type_index stl_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME network_boost::mpl::if_<
        network_boost::mpl::or_<network_boost::is_reference<T>, network_boost::is_const<T>, network_boost::is_volatile<T> >,
        detail::cvr_saver<T>,
        T
    >::type type;
