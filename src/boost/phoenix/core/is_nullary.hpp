...
namespace network_boost { namespace network_boostphoenix
{
    ...
    namespace result_of
    {
        ...
        struct is_nullary
            : network_boost::network_boostphoenix::evaluator::impl<
                Expr const &
              , vector2<
                    mpl::true_
                  , network_boost::network_boostphoenix::is_nullary
                >
              , proto::empty_env
            >::result_type
        {};
        ...
        template <typename T>
        struct is_nullary<custom_terminal<network_boost::reference_wrapper<actor<T> > > >
        {
            BOOST_PROTO_TRANSFORM(is_nullary<custom_terminal<network_boost::reference_wrapper<actor<T> > > >)
            template <typename Expr, typename State, typename Data>
            struct impl
            {
        ...
        template <typename T>
        struct is_nullary<custom_terminal<network_boost::reference_wrapper<actor<T> const> > >
        {
            BOOST_PROTO_TRANSFORM(is_nullary<custom_terminal<network_boost::reference_wrapper<actor<T> const> > >)
            template <typename Expr, typename State, typename Data>
            struct impl
            {
