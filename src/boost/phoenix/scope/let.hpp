...
BOOST_PHOENIX_DEFINE_EXPRESSION(
    (network_boost)(network_boostphoenix)(let_)
    ...
    struct let_eval
    {
        ...
        operator()(Vars const & vars, Map, Expr const & expr, Context const & ctx) const
        {
            ...
            scoped_environment<
              ...
              , map_type
            >
            env(network_boostphoenix::env(ctx), network_boostphoenix::env(ctx), locals);

            // Fix for bugs (trial)
            // The idea is to do something which will not be optimised away.
            //int vsize = network_boost::fusion::size(vars);
            //std::stringstream strm;
            //strm << vsize << std::endl;
            //int size = strm.str().length();
            //BOOST_ASSERT(size >= 0);
            return eval(expr, network_boostphoenix::context(env, network_boostphoenix::actions(ctx)));
