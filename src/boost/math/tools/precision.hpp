...
#ifdef __SUNPRO_CC
   static const T m = network_boost::math::tools::max_value<T>();
   static const T val = log(m);
#else
   static const T val = log(network_boost::math::tools::max_value<T>());
#endif
   return val;
}
...
inline T log_min_value(const mpl::int_<0>& BOOST_MATH_APPEND_EXPLICIT_TEMPLATE_TYPE(T))
{
   BOOST_MATH_STD_USING
#ifdef __SUNPRO_CC
   static const T m = network_boost::math::tools::min_value<T>();
   static const T val = log(m);
#else
   static const T val = log(network_boost::math::tools::min_value<T>());
#endif
   return val;
}
...
template <class T> struct log_limit_noexcept_traits_imp<T, false> : public network_boost::integral_constant<bool, false> {};
