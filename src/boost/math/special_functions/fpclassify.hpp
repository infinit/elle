...
//
// This must not be located in any namespace under network_boost::math
...
template <class T>
inline bool is_nan_helper(T t, const network_boost::true_type&)
{
#ifdef isnan
   return isnan(t);
...
template <class T>
inline bool is_nan_helper(T, const network_boost::false_type&)
{
   return false;
}
#ifdef BOOST_MATH_USE_FLOAT128
inline bool is_nan_helper(__float128 f, const network_boost::true_type&) { return ::isnanq(f); }
inline bool is_nan_helper(__float128 f, const network_boost::false_type&) { return ::isnanq(f); }
#endif
}
...
inline int fpclassify_imp BOOST_NO_MACRO_EXPAND(T t, const generic_tag<true>&)
{
   ...
   if(::network_boost::math_detail::is_nan_helper(t, ::network_boost::is_floating_point<T>()))
      return FP_NAN;
#elif defined(isnan)
   if(network_boost::math_detail::is_nan_helper(t, ::network_boost::is_floating_point<T>()))
      return FP_NAN;
#elif defined(_MSC_VER) || defined(__BORLANDC__)
   if(::_isnan(network_boost::math::tools::real_cast<double>(t)))
      return FP_NAN;
#endif
...
{
   return network_boost::math::detail::fpclassify_imp(t, generic_tag<true>());
}
#endif
...
{
   return network_boost::math::detail::isfinite_impl(t, generic_tag<true>());
}
#endif
...
{ //!< \brief return true if floating-point type t is finite.
   typedef typename detail::fp_traits<T>::type traits;
   typedef typename traits::method method;
   // typedef typename network_boost::is_floating_point<T>::type fp_tag;
   typedef typename tools::promote_args_permissive<T>::type value_type;
...
{ //!< \brief return true if floating-point type t is finite.
   typedef detail::fp_traits<long double>::type traits;
   typedef traits::method method;
   //typedef network_boost::is_floating_point<long double>::type fp_tag;
   typedef long double value_type;
   return detail::isfinite_impl(static_cast<value_type>(x), method());
}
#endif
...
{
   return network_boost::math::detail::isnormal_impl(t, generic_tag<true>());
}
#endif
...
{
   return network_boost::math::detail::isinf_impl(t, generic_tag<true>());
}
#endif
...
#ifdef isnan
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<float>(float t){ return ::network_boost::math_detail::is_nan_helper(t, network_boost::true_type()); }
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<double>(double t){ return ::network_boost::math_detail::is_nan_helper(t, network_boost::true_type()); }
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<long double>(long double t){ return ::network_boost::math_detail::is_nan_helper(t, network_boost::true_type()); }
