...
protected:
   ...
   const ::network_boost::regex_traits_wrapper<traits>&  
   ...
   network_boost::uintmax_t              m_bad_repeats;        // bitmask of repeats we can't deduce a startmap for;
   ...
   : m_pdata(data), m_traits(*(data->m_ptraits)), m_last_state(0), m_repeater_id(0), m_has_backrefs(false), m_backrefs(0), m_has_recursions(false)
{
   ...
   m_pdata->m_status = ::network_boost::regex_constants::error_ok;
   ...
   const basic_char_set<charT, traits>& char_set, mpl::false_*)
{
   ...
   // fill in the basics:
   //
   result->csingles = static_cast<unsigned int>(::network_boost::BOOST_REGEX_DETAIL_NS::distance(char_set.singles_begin(), char_set.singles_end()));
   result->cranges = static_cast<unsigned int>(::network_boost::BOOST_REGEX_DETAIL_NS::distance(char_set.ranges_begin(), char_set.ranges_end())) / 2;
   result->cequivalents = static_cast<unsigned int>(::network_boost::BOOST_REGEX_DETAIL_NS::distance(char_set.equivalents_begin(), char_set.equivalents_end()));
               ...
               if(idx >= 10000)
               {
                  ...
                  if(idx <= 0)
                  {
                     ...
                     if(0 == this->m_pdata->m_status) // update the error code if not already set
                        this->m_pdata->m_status = network_boost::regex_constants::error_bad_pattern;
                     //
                     ...
                     if(0 == (this->flags() & regex_constants::no_except))
                     {
                        ...
                        network_boost::regex_error e(message, network_boost::regex_constants::error_bad_pattern, 0);
                        e.raise();
                     }
                  }
               }
   ...
   case syntax_element_long_set_rep:
      {
         ...
         static const network_boost::uintmax_t one = 1uL;
