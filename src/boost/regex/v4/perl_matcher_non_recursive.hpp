...
bool perl_matcher<BidiIterator, Allocator, traits>::match_all_states()
{
   static matcher_proc_type const s_match_vtable[34] = 
   {
      ...
      //(::network_boost::is_random_access_iterator<BidiIterator>::value ? &perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast : &perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_slow),
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast()
{
   ...
   const re_repeat* rep = static_cast<const re_repeat*>(pstate);
   bool greedy = (rep->greedy) && (!(m_match_flags & regex_constants::match_any) || m_independent);   
   unsigned count = static_cast<unsigned>((std::min)(static_cast<unsigned>(::network_boost::BOOST_REGEX_DETAIL_NS::distance(position, last)), static_cast<unsigned>(greedy ? rep->max : rep->min)));
   if(rep->min > count)
   {
      position = last;
...
#pragma option push -w-8008 -w-8066 -w-8004
   ...
   if(::network_boost::is_random_access_iterator<BidiIterator>::value)
   {
      ...
      std::size_t len = (desired == (std::numeric_limits<std::size_t>::max)()) ? 0u : ::network_boost::BOOST_REGEX_DETAIL_NS::distance(position, last);
      if(desired >= len)
         end = last;
      else
      ...
      count = (unsigned)::network_boost::BOOST_REGEX_DETAIL_NS::distance(origin, position);
   }
   else
   {
...
bool perl_matcher<BidiIterator, Allocator, traits>::unwind_paren(bool have_match)
{
   ...
   network_boost::BOOST_REGEX_DETAIL_NS::inplace_destroy(pmp);
...
{
   network_boost::BOOST_REGEX_DETAIL_NS::inplace_destroy(m_backup_state++);
   pstate = 0;   // nothing left to search
   return false; // end of stack nothing more to search
}
...
inline void perl_matcher<BidiIterator, Allocator, traits>::destroy_single_repeat()
{
   ...
   network_boost::BOOST_REGEX_DETAIL_NS::inplace_destroy(p++);
   m_backup_state = p;
}
