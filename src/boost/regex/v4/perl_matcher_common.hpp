...
{ 
   ...
   if(e.empty())
   {
      ...
      network_boost::throw_exception(ex);
   }
   pstate = 0;
   m_match_flags = f;
...
void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(std::random_access_iterator_tag*)
{
   ...
   std::ptrdiff_t dist = network_boost::BOOST_REGEX_DETAIL_NS::distance(base, last);
   if(dist == 0)
      dist = 1;
...
{
   ::network_boost::BOOST_REGEX_DETAIL_NS::concrete_protected_call
...
#pragma warning(disable:4127)
   ...
   {
      std::ptrdiff_t maxlen = ::network_boost::BOOST_REGEX_DETAIL_NS::distance(backstop, position);
