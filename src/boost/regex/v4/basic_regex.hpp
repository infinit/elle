...
{
   std::size_t r = network_boost::hash_range(i, j);
...
struct regex_data : public named_subexpressions
{
   ...
   regex_data(const ::network_boost::shared_ptr<
      ::network_boost::regex_traits_wrapper<traits> >& t) 
      : m_ptraits(t), m_expression(0), m_expression_len(0), m_disable_match_any(false) {}
   regex_data() 
      : m_ptraits(new ::network_boost::regex_traits_wrapper<traits>()), m_expression(0), m_expression_len(0), m_disable_match_any(false) {}

   ::network_boost::shared_ptr<
      ::network_boost::regex_traits_wrapper<traits>
...
public:
   ...
   basic_regex_implementation(const ::network_boost::shared_ptr<
      ::network_boost::regex_traits_wrapper<traits> >& t)
   ...
   const ::network_boost::regex_traits_wrapper<traits>& get_traits()const
   {
...
public:
   ...
   {
      if(!m_pimpl.get())
         network_boost::throw_exception(std::logic_error("Can't access subexpressions in an invalid regex."));
   ...
   network_boost::shared_ptr<BOOST_REGEX_DETAIL_NS::named_subexpressions > get_named_subs()const
   {
      return m_pimpl;
   }
