...
public: 
   ...
   difference_type position(size_type sub = 0) const
   {
      ...
      if(sub < m_subs.size())
      {
         ...
         if(s.matched || (sub == 2))
         {
            return ::network_boost::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)(m_base), (BidiIterator)(s.first));
         }
      }
                         ...
                         const RegexT& re) const
   {
      if(m_is_singular)
         raise_logic_error();
      typedef ::network_boost::regex_traits_wrapper<typename RegexT::traits_type> traits_type;
   ...
   void BOOST_REGEX_CALL set_named_subs(network_boost::shared_ptr<named_sub_type> subs)
   {
      m_named_subs = subs;
   }
...
private:
   ...
   {
      std::logic_error e("Attempt to access an uninitialzed network_boost::match_results<> class.");
   ...
   network_boost::shared_ptr<named_sub_type> m_named_subs;     // Shared copy of named subs in the regex object
...
void BOOST_REGEX_CALL match_results<BidiIterator, Allocator>::maybe_assign(const match_results<BidiIterator, Allocator>& m)
{
   ...
   for(i = 0; i < size(); ++i, ++p1, ++p2)
   {
      ...
      base1 = ::network_boost::BOOST_REGEX_DETAIL_NS::distance(l_base, p1->first);
      base2 = ::network_boost::BOOST_REGEX_DETAIL_NS::distance(l_base, p2->first);
      BOOST_ASSERT(base1 >= 0);
      BOOST_ASSERT(base2 >= 0);
      if(base1 < base2) return;
      if(base2 < base1) break;

      len1 = ::network_boost::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)p1->first, (BidiIterator)p1->second);
      len2 = ::network_boost::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)p2->first, (BidiIterator)p2->second);
      BOOST_ASSERT(len1 >= 0);
      BOOST_ASSERT(len2 >= 0);
