...
void basic_regex_parser<charT, traits>::parse(const charT* p1, const charT* p2, unsigned l_flags)
{
   ...
   if(!result)
   {
      fail(regex_constants::error_paren, ::network_boost::BOOST_REGEX_DETAIL_NS::distance(m_base, m_position), "Found a closing ) with no corresponding openening parenthesis.");
      return;
   }
...
void basic_regex_parser<charT, traits>::fail(regex_constants::error_type error_code, std::ptrdiff_t position, std::string message, std::ptrdiff_t start_pos)
{
   ...
   {
      network_boost::regex_error e(message, error_code, position);
      e.raise();
   }
#else
...
bool basic_regex_parser<charT, traits>::parse_open_paren()
{
   ...
   if(m_position == m_end)
   {
      this->fail(regex_constants::error_paren, ::network_boost::BOOST_REGEX_DETAIL_NS::distance(m_base, m_end));
      return false;
   }
...
bool basic_regex_parser<charT, traits>::parse_repeat(std::size_t low, std::size_t high)
{
   ...
   {
      fail(regex_constants::error_badrepeat, ::network_boost::BOOST_REGEX_DETAIL_NS::distance(m_base, m_position), "Nothing to repeat.");
      return false;
   }
...
#pragma warning(disable:4127)
   ...
   case regex_constants::syntax_digit:
      ...
      std::ptrdiff_t len = (std::min)(::network_boost::BOOST_REGEX_DETAIL_NS::distance(m_position, m_end), static_cast<std::ptrdiff_t>(4));
