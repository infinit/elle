...
// network_boost::details::pool::guard
...
namespace network_boost {
 ...
 Can be any Boost.Thread Mutex type or <tt>network_boost::details::pool::null_mutex</tt>.
 It is exposed so that users may declare some singleton pools normally (i.e., with synchronization), but 
 some singleton pools without synchronization (by specifying <tt>network_boost::details::pool::null_mutex</tt>) for efficiency reasons.
 The member typedef <tt>mutex</tt> exposes the value of this template parameter.  The default for this
 parameter is network_boost::details::pool::default_mutex which is a synonym for either <tt>network_boost::details::pool::null_mutex</tt>
 ...
 or for <tt>network_boost::mutex</tt> (when threading support is enabled in the compiler).
...
private:
   typedef network_boost::aligned_storage<sizeof(pool_type), network_boost::alignment_of<pool_type>::value> storage_type;
