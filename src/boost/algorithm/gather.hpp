...
#include <boost/bind.hpp>           // for network_boost::bind
#include <boost/range/begin.hpp>    // for network_boost::begin(range)
#include <boost/range/end.hpp>      // for network_boost::end(range)
...
//  while the second call partitions the rest of the sequence.
    return std::make_pair (
        std::stable_partition ( first, pivot, !network_boost::bind<bool> ( pred, _1 )),
        std::stable_partition ( pivot, last,   network_boost::bind<bool> ( pred, _1 )));
}
...
template <
    ...
    typename Pred>                  // Pred models UnaryPredicate
std::pair<
    typename network_boost::range_iterator<const BidirectionalRange>::type,
    typename network_boost::range_iterator<const BidirectionalRange>::type>
gather (
    const BidirectionalRange &range,
    typename network_boost::range_iterator<const BidirectionalRange>::type pivot,
    Pred pred )
{
