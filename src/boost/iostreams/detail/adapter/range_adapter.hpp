...
#include <boost/detail/iterator.hpp>             // network_boost::iterator_traits.
...
// Description: Device based on an instance of network_boost::iterator_range.
// Template parameters:
//     Mode - A mode tag.
...
private:
    ...
    typedef network_boost::detail::iterator_traits<iterator>  iter_traits;
...
struct range_adapter_impl<std::forward_iterator_tag> {
        ...
        (Iter& cur, Iter& last, const Ch* s, std::streamsize n)
    {
        ...
        if (cur == last && n > 0)
            network_boost::throw_exception(write_area_exhausted());
        return n;
    }
};
          ...
          BOOST_IOS::seekdir way )
    {
        ...
        case BOOST_IOS::beg:
            if (off > last - first || off < 0)
                network_boost::throw_exception(bad_seek());
            cur = first + off;
            break;
        case BOOST_IOS::cur:
            {
