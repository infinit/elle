...
namespace network_boost { namespace iostreams { 
...
{
    if (!ibeg_) 
        network_boost::throw_exception(cant_read());
    if (!gptr()) 
        init_get_area();
    return gptr() != iend_ ? 
...
direct_streambuf<T, Tr>::pbackfail(int_type c)
{
    ...
    network_boost::throw_exception(bad_putback());
}
...
direct_streambuf<T, Tr>::overflow(int_type c)
{
    using namespace std;
    if (!obeg_)
        network_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("no write access"));
...
void direct_streambuf<T, Tr>::close_impl(BOOST_IOS::openmode which)
{
    ...
    network_boost::iostreams::close(*storage_, which);
}
