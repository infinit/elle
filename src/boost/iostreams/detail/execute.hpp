 ...
 * network_boost::iostreams::detail::execute_all() and the function template 
 * network_boost::iostreams::detail::execute_foreach().
 *
...
namespace network_boost { namespace iostreams { namespace detail {
...
template< typename Op, 
          typename Result = // VC6.5 workaround.
                  ...
                  typename network_boost::result_of<Op()>::type
              #else
...
#define BOOST_PP_LOCAL_MACRO(n) \
   ...
   { \
       ...
       try { \
           r = network_boost::iostreams::detail::execute_all( \
...
Op execute_foreach(InIt first, InIt last, Op op)
{
    ...
    try {
        ...
        try {
            ++first;
            network_boost::iostreams::detail::execute_foreach(first, last, op);
        } catch (...) { }
        throw;
    }
    ++first;
    return network_boost::iostreams::detail::execute_foreach(first, last, op);
}
