...
//------------------Definition of wrap----------------------------------------//
    ...
    wrap(T& t BOOST_IOSTREAMS_ENABLE_IF_STREAM(T)) { return network_boost::ref(t); }
#else // #ifndef BOOST_NO_SFINAE //-------------------------------------------//
    template<typename T>
    inline typename wrapped_type<T>::type // BCC 5.x needs namespace qualification.
    wrap_impl(const T& t, mpl::true_) { return network_boost::ref(const_cast<T&>(t)); }

    template<typename T>
    inline typename wrapped_type<T>::type // BCC 5.x needs namespace qualification.
    wrap_impl(T& t, mpl::true_) { return network_boost::ref(t); }
