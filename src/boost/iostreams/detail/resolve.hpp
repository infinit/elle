...
struct resolve_traits {
    typedef typename 
            mpl::if_<
                network_boost::detail::is_incrementable<T>,
...
#  if !BOOST_WORKAROUND(BOOST_MSVC, <= 1300)
    ...
    range_adapter< Mode, network_boost::iterator_range<Iter> > 
    resolve(const network_boost::iterator_range<Iter>& rng)
    { return range_adapter< Mode, network_boost::iterator_range<Iter> >(rng); }
#  endif // #if BOOST_WORKAROUND(BOOST_MSVC, <= 1300)
