...
#define BOOST_IOSTREAMS_PIPABLE(filter, arity) \
    ...
    ::network_boost::iostreams::pipeline< \
        ::network_boost::iostreams::detail::pipeline_segment< \
    ...
    > operator|( const filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T)& f, \
                 const Component& c ) \
    { \
        typedef ::network_boost::iostreams::detail::pipeline_segment< \
                    filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T) \
                > segment; \
        return ::network_boost::iostreams::pipeline<segment, Component> \
                   (segment(f), c); \
    } \
    /**/
