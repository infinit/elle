...
{
   typedef ::network_boost::movelib::unique_ptr<T> t_is_not_array;
};

template<class T>
struct unique_ptr_if<T[]>
{
   typedef ::network_boost::movelib::unique_ptr<T[]> t_is_array_of_unknown_bound;
};
...
inline BOOST_MOVE_DOC1ST(unique_ptr<T>, 
   typename ::network_boost::move_upmu::unique_ptr_if<T>::t_is_not_array)
      make_unique(BOOST_FWD_REF(Args)... args)
{  return unique_ptr<T>(new T(::network_boost::forward<Args>(args)...));  }
...
{  return unique_ptr<T>(new (*network_boost::move_upmu::nothrow_holder<>::pnothrow)T(::network_boost::forward<Args>(args)...));  }
   ...
   #define BOOST_MOVE_MAKE_UNIQUE_CODE(N)\
      ...
      {  return unique_ptr<T>( new (*network_boost::move_upmu::nothrow_holder<>::pnothrow)T ( BOOST_MOVE_FWD##N ) );  }\
      //
...
{
    return unique_ptr<T>(new (*network_boost::move_upmu::nothrow_holder<>::pnothrow)T);
}
...
template<class T>
inline BOOST_MOVE_DOC1ST(unique_ptr<T>, 
   typename ::network_boost::move_upmu::unique_ptr_if<T>::t_is_array_of_unknown_bound)
      make_unique(std::size_t n)
{
    typedef typename ::network_boost::move_upmu::remove_extent<T>::type U;
...
{
    typedef typename ::network_boost::move_upmu::remove_extent<T>::type U;
    return unique_ptr<T>(new (*network_boost::move_upmu::nothrow_holder<>::pnothrow)U[n]());
}
...
inline BOOST_MOVE_DOC1ST(unspecified, 
   typename ::network_boost::move_upmu::unique_ptr_if<T>::t_is_array_of_known_bound)
