  ...
  convert_to_lower(std::string inp)
  {
    ...
    month_str_to_ushort(std::string const& s) {
      if((s.at(0) >= '0') && (s.at(0) <= '9')) {
        return network_boost::lexical_cast<unsigned short>(s);
      }
      else {
    ...
    parse_date(const std::string& s, int order_spec = ymd_order_iso) {
      ...
      typedef network_boost::char_separator<char, traits_type> char_separator_type;
      ...
      for(tokenizer_iterator beg=tok.begin();
        ...
        switch(spec_str.at(pos)) {
          case 'y':
          {
            year = network_boost::lexical_cast<unsigned short>(*beg);
            break;
          }
          case 'm':
          {
          ...
          case 'd':
          {
            day = network_boost::lexical_cast<unsigned short>(*beg);
            break;
          }
          default: break;
        } //switch
    ...
    parse_undelimited_date(const std::string& s) {
      ...
      network_boost::offset_separator osf(offsets, offsets+3, false, false);

      typedef typename network_boost::tokenizer<network_boost::offset_separator,
                                        std::basic_string<char>::const_iterator,
      ...
      for(typename tokenizer_type::iterator ti=tok.begin(); ti!=tok.end();++ti) {
        unsigned short i = network_boost::lexical_cast<unsigned short>(*ti);
        switch(pos) {
        case 0: y = i; break;
        case 1: m = i; break;
        case 2: d = i; break;
    ...
    from_simple_string_type(const std::basic_string<charT>& s){
      ...
      typedef typename network_boost::char_separator<charT, traits_type> char_separator;
      typedef typename network_boost::tokenizer<char_separator,
