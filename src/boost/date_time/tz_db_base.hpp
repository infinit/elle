...
namespace network_boost {
  namespace date_time {
    ...
    public:
      ...
      typedef network_boost::date_time::dst_adjustment_offsets<time_duration_type> dst_adjustment_offsets;
      ...
      void load_from_file(const std::string& pathspec)
      {
        ...
        if(!ifs){
          network_boost::throw_exception(data_not_accessible(pathspec));
        }
      ...
      bool add_record(const string_type& region, 
                      network_boost::shared_ptr<time_zone_base_type> tz)
      {
      ...
      network_boost::shared_ptr<time_zone_base_type> 
      time_zone_from_region(const string_type& region) const 
      {
        ...
        if(record == m_zone_map.end()){
          return network_boost::shared_ptr<time_zone_base_type>(); //null pointer
        }
    ...
    private:
      typedef std::map<string_type, network_boost::shared_ptr<time_zone_base_type> > map_type;
      map_type m_zone_map;
      ...
      {
        typedef network_boost::char_separator<char_type, std::char_traits<char_type> > char_separator_type;
        typedef network_boost::tokenizer<char_separator_type,
        ...
        if ( std::distance ( tokens.begin(), tokens.end ()) != 3 ) {
          ...
          msg << "Expecting 3 fields, got " 
              ...
              << " fields in line: " << rule;
          network_boost::throw_exception(bad_field_count(msg.str()));
        }
      ...
      bool parse_string(string_type& s)
      {
        ...
        typedef network_boost::token_iterator_generator<network_boost::escaped_list_separator<char_type>, string_type::const_iterator, string_type >::type token_iter_type;

        token_iter_type i = network_boost::make_token_iterator<string_type>(s.begin(), s.end(),network_boost::escaped_list_separator<char_type>());
        ...
        network_boost::shared_ptr<rule_type> rules;
        ...
        if(has_dst){
          ...
          rules = 
            network_boost::shared_ptr<rule_type>(parse_rules(result[START_DATE_RULE],
                                                     result[END_DATE_RULE]));
        }
        string_type id(result[ID]);
        network_boost::shared_ptr<time_zone_base_type> zone(new time_zone_type(names, utc_offset, adjust, rules));
