...
namespace local_time {
   ...
   * required to be in the form of a network_boost::shared_ptr<time_zone_base>.
   */
  ...
  class local_date_time_base :  public date_time::base_time<utc_time_,
                                                            network_boost::posix_time::posix_time_system> {
  public:
    ...
    local_date_time_base(utc_time_type t,
                         network_boost::shared_ptr<tz_type> tz) :
    ...
    local_date_time_base(date_type d,
      ...
      zone_(tz)
    {
      if(tz != network_boost::shared_ptr<tz_type>() && tz->has_dst()){
        ...
        if(result == invalid_time_label){
          ...
          ss << "time given: " << d << ' ' << td;
          network_boost::throw_exception(time_label_invalid(ss.str()));
        }
          ...
          network_boost::throw_exception(dst_not_valid(ss.str()));
        }
      ...
      zone_(tz)
    {
      ...
      if(result == ambiguous) {
        if(calc_option == EXCEPTION_ON_ERROR){
          std::ostringstream ss;
          ss << "time given: " << d << ' ' << td;
          network_boost::throw_exception(ambiguous_result(ss.str()));
        }
        else{ // NADT on error
    ...
    explicit local_date_time_base(const network_boost::date_time::special_values sv,
                                  network_boost::shared_ptr<tz_type> tz = network_boost::shared_ptr<tz_type>()) :
      date_time::base_time<utc_time_type, time_system_type>(utc_time_type(sv)),
      zone_(tz)
    {}

    //! returns time zone associated with calling instance
    network_boost::shared_ptr<tz_type> zone() const
    {
      return zone_;
    }
    ...
    bool is_dst() const
    {
      if(zone_ != network_boost::shared_ptr<tz_type>() && zone_->has_dst() && !this->is_special()) {
    ...
    {
      if(zone_ != network_boost::shared_ptr<tz_type>()){
    ...
    local_date_time_base local_time_in(network_boost::shared_ptr<tz_type> new_tz,
    ...
    /*bool operator==(const date_time::base_time<network_boost::posix_time::ptime,network_boost::posix_time::posix_time_system>& rhs) const
    { // fails due to rhs.time_ being protected
      return date_time::base_time<network_boost::posix_time::ptime,network_boost::posix_time::posix_time_system>::operator==(rhs);
  ...
  private:
    network_boost::shared_ptr<tz_type> zone_;
    //bool is_dst_;

    /*! Adjust the passed in time to UTC?
     */
    utc_time_type construction_adjustment(utc_time_type t,
                                          network_boost::shared_ptr<tz_type> z,
                                          bool dst_flag)
    {
      if(z != network_boost::shared_ptr<tz_type>()) {
