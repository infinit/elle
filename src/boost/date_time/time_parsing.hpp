...
namespace date_time {
  ...
  str_from_delimited_time_duration(const std::basic_string<char_type>& s)
  {
    ...
    int hour =0; 
    bool is_neg = (s.at(0) == '-');
    network_boost::int64_t fs=0;
    int pos = 0;
      
    typedef typename std::basic_string<char_type>::traits_type traits_type;
    typedef network_boost::char_separator<char_type, traits_type> char_separator_type;
    typedef network_boost::tokenizer<char_separator_type,
                             typename std::basic_string<char_type>::const_iterator,
                             std::basic_string<char_type> > tokenizer;
    typedef typename network_boost::tokenizer<char_separator_type,
                             typename std::basic_string<char_type>::const_iterator,
    ...
    for(tokenizer_iterator beg=tok.begin(); beg!=tok.end();++beg){
      switch(pos) {
      case 0: {
        hour = network_boost::lexical_cast<int>(*beg);
        break;
      }
      case 1: {
        min = network_boost::lexical_cast<unsigned short>(*beg);
        break;
      }
      case 2: {
        sec = network_boost::lexical_cast<unsigned short>(*beg);
        break;
      };
      case 3: {
        ...
        if(digits >= precision) {
          // drop excess digits
          fs = network_boost::lexical_cast<network_boost::int64_t>(beg->substr(0, precision));
        }
        else {
          fs = network_boost::lexical_cast<network_boost::int64_t>(*beg);
        }
#endif
        if(digits < precision){
  ...
  parse_undelimited_time_duration(const std::string& s)
  {
    ...
    bool wrap_off = false;
    bool ret_part = true;
    network_boost::offset_separator osf(offsets, offsets+4, wrap_off, ret_part); 
    typedef network_boost::tokenizer<network_boost::offset_separator,
    ...
    for(tokenizer_iterator ti=tok.begin(); ti!=tok.end();++ti){
      switch(pos) {
        case 0: 
          {
            hours = network_boost::lexical_cast<int>(*ti); 
            break;
          }
        case 1: 
          {
            min = network_boost::lexical_cast<short>(*ti); 
            break;
          }
        case 2: 
          {
            sec = network_boost::lexical_cast<short>(*ti); 
            break;
          }
        case 3:
          {
            ...
            if(digits >= precision) {
              // drop excess digits
              fs = network_boost::lexical_cast<network_boost::int64_t>(char_digits.substr(0, precision));
            }
            else if(digits == 0) {
              fs = 0; // lexical_cast doesn't like empty strings
            }
            else {
              fs = network_boost::lexical_cast<network_boost::int64_t>(char_digits);
            }
#endif
            if(digits < precision){
              // trailing zeros get dropped from the string, 
