...
namespace network_boost { namespace spirit { namespace x3
{
    template <
        typename Encoding
    ...
    struct symbols_parser : parser<symbols_parser<Encoding, T, Lookup>>
    {
        ...
        symbols_parser(Symbols const& syms, std::string const& name = "symbols")
          ...
          , name_(name)
        {
            typename range_const_iterator<Symbols>::type si = network_boost::begin(syms);
            while (si != network_boost::end(syms))
                add(*si++);
        }
        ...
        {
            typename range_const_iterator<Symbols>::type si = network_boost::begin(syms);
            typename range_const_iterator<Data>::type di = network_boost::begin(data);
            while (si != network_boost::end(syms))
                add(*si++, *di++);
        }
          ...
          , name_(name)
        {
            ...
            typename range_const_iterator<symbols_t>::type si = network_boost::begin(syms);
            for (;si != network_boost::end(syms); ++si)
