...
namespace network_boost { namespace spirit { namespace x3 { namespace testing
{
    namespace fs = network_boost::filesystem;
   ...
   {
       network_boost::regex e(re_prefix + std::string("(.*?)") + re_suffix);
       network_boost::match_results<Iterator> what;
       if (network_boost::regex_search(
            first, last, what, e
          , network_boost::match_default | network_boost::match_continuous))
        {
            re = what[1].str();
            first = what[0].second;
            return true;
    ...
    )
    {
       ...
       while (in_first != in_last && tem_first != tem_last)
       {
            ...
            {
                network_boost::match_results<iter_t> what;
                network_boost::regex e(re);
                if (!network_boost::regex_search(
    ...
    {
        network_boost::filesystem::ifstream file(p);
        if (!file)
            return "";
