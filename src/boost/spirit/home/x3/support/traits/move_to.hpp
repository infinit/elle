...
namespace network_boost { namespace spirit { namespace x3 { namespace traits
{
    ...
    namespace detail
    {
        ...
        template <typename Iterator>
        inline void
        move_to(Iterator first, Iterator last, network_boost::iterator_range<Iterator>& rng, container_attribute)
        {
            rng = {first, last};
        }
    }
    ...
    {
        if (network_boost::addressof(src) != network_boost::addressof(dest))
            dest = std::move(src);
    }
