...
#include <boost/detail/iterator.hpp> // for network_boost::detail::iterator_traits
...
namespace network_boost { namespace spirit { namespace traits
{
    ...
    template <typename T>
    struct is_container<network_boost::optional<T> >
      : is_container<T>
    {};

#if !defined(BOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES)
    template<typename T>
    struct is_container<network_boost::variant<T> >
      : is_container<T>
    {};

    template<typename T0, typename T1, typename ...TN>
    struct is_container<network_boost::variant<T0, T1, TN...> >
      : mpl::bool_<is_container<T0>::value ||
            is_container<network_boost::variant<T1, TN...> >::value>
    {};
...
#define BOOST_SPIRIT_IS_CONTAINER(z, N, data)                                 \
    ...
    template <>
    struct is_container<network_boost::detail::variant::void_>
      : mpl::false_
    {};
...
#undef BOOST_SPIRIT_IS_CONTAINER
    ...
    template <typename T>
    struct container_value<network_boost::optional<T> >
      : container_value<T>
    {};
    ...
    template <typename T>
    struct optional_attribute<network_boost::optional<T> >
    {
        typedef T const& type;

        static type call(network_boost::optional<T> const& val)
        {
            return network_boost::get<T>(val);
        }

        static bool is_valid(network_boost::optional<T> const& val)
        {
            return !!val;
        }
    };
    ...
    {
        static bool call(network_boost::optional<Container>& c, T const& val)
        {
            if (!c)
                c = Container();
            return push_back(network_boost::get<Container>(c), val);
        }
    };
    ...
    {
        typedef typename network_boost::detail::iterator_traits<Iterator>::reference type;
...
namespace network_boost { namespace spirit { namespace result_of
{
    ...
    template <typename T>
    struct optional_value<network_boost::optional<T> >
    {
        typedef T type;
    };

    template <typename T>
    struct optional_value<network_boost::optional<T> const>
    {
        typedef T const type;
    };
