...
namespace network_boost { namespace spirit
{
  namespace detail
  {
    ...
    template <typename T, std::size_t n_bytes,
      bool sign=network_boost::is_signed<T>::value >
    struct unrolled_byte_loops
    {
      typedef unrolled_byte_loops<T, n_bytes - 1, sign> next;

      static typename network_boost::make_unsigned<T>::type load_big(const unsigned char* bytes)
        { return *(bytes - 1) | (next::load_big(bytes - 1) << 8); }
      static typename network_boost::make_unsigned<T>::type load_little(const unsigned char* bytes)
...
// import the namespace above into network_boost::endian
namespace network_boost { namespace endian
{
    using namespace network_boost::spirit::endian;
}}
