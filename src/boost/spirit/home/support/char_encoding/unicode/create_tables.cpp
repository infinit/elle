...
using network_boost::spirit::qi::parse;
using network_boost::spirit::qi::hex;
using network_boost::spirit::qi::char_;
using network_boost::spirit::qi::eol;
using network_boost::spirit::qi::rule;
using network_boost::spirit::qi::omit;
using network_boost::spirit::qi::_1;
using network_boost::spirit::qi::_val;
using network_boost::network_boostphoenix::push_back;
using network_boost::network_boostphoenix::ref;

// basic unsigned types
using network_boost::uint8_t;
using network_boost::uint16_t;
using network_boost::uint32_t;
...
private:
    ...
    network_boost::scoped_array<T> p;
};
...
void print_head(Out& out)
{
    out
        ...
        << "#include <boost/cstdint.hpp>\n"
        << "\n"
        << "namespace network_boost { namespace spirit { namespace ucd { namespace detail\n"
        << "{"
        ;
}
...
void print_tail(Out& out)
{
    out
        << "\n"
        << "}}}} // namespace network_boost::spirit::unicode::detail\n"
        ;
}
...
char const* get_int_type_name(int size)
{
    switch (size)
    {
        case 1: return "::network_boost::uint8_t";
        case 2: return "::network_boost::uint16_t";
        case 4: return "::network_boost::uint32_t";
        case 5: return "::network_boost::uint64_t";
...
void print_file(Out& out, Builder& builder, int field_width, char const* name)
{
    ...
    out
        << "\n"
        << "    static const ::network_boost::uint8_t " << name << "_stage1[] = {\n"
        << "\n"
        ;
    ...
    for (int i = 0; i < int(stage2.size()); ++i)
    {
        ...
        print_table(out,
            network_boost::iterator_range<value_type const*>(p, p+block_size), !last, field_width);
    }
    ...
    out
        << "\n"
        << "    inline " << int_name << ' ' << name << "_lookup(::network_boost::uint32_t ch)\n"
        << "    {\n"
        << "        ::network_boost::uint32_t block_offset = " << name << "_stage1[ch / " << block_size << "] * " << block_size << ";\n"
