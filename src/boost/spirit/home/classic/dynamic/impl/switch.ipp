...
struct default_delegate_parse<N, true, HasDefault> {
        ...
        ScannerT const &scan, typename ScannerT::iterator_t const save)
    {
        ...
        BOOST_STATIC_ASSERT((!network_boost::is_same<DefaultT, nothing_parser>::value));
        ...
        parser<compound_case_parser<LeftT, RightT, IsDefault> > >
{
    ...
    BOOST_STATIC_CONSTANT(bool, is_epsilon = (
        is_default &&
            network_boost::is_same<typename RightT::subject_t, epsilon_parser>::value
    ));
...
struct cond_actor {
    ...
    struct result
    {
        typedef typename ::network_boostphoenix::actor_result<ActorT, ::network_boostphoenix::tuple<> >::type
            type;
    };
...
template <typename ActorT>
struct make_cond_functor< ::network_boostphoenix::actor<ActorT> > {

    static cond_actor< ::network_boostphoenix::actor<ActorT> >
    do_(::network_boostphoenix::actor<ActorT> const &actor)
    {
        return cond_actor< ::network_boostphoenix::actor<ActorT> >(actor);
    }
};
