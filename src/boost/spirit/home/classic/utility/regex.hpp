...
struct rxstrlit : public parser<rxstrlit<CharT> > {
    ...
    {
    //  Due to limitations in the network_boost::regex library the iterators wrapped in
    //  the ScannerT object should be at least bidirectional iterators. Plain
    //  forward iterators do not work here.
        typedef typename ScannerT::iterator_t iterator_t;
        typedef
            typename network_boost::detail::iterator_traits<iterator_t>::iterator_category
            iterator_category;

        BOOST_STATIC_ASSERT((
            network_boost::is_convertible<iterator_category,
