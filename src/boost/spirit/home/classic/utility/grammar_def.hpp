...
namespace impl {
    ...
    private:
        ...
        BOOST_STATIC_ASSERT((!network_boost::is_same<T0, same>::value));

        typedef typename network_boost::mpl::if_c<
                    network_boost::is_same<T, same>::value,
                    T0 const *,
                    T const *
                >::type
            ptr_type;
    ...
    public:
        // If the type in question is network_boostphoenix::nil_t, then the returned type
        // is still network_boostphoenix::nil_t, otherwise a constant pointer type to the
        // inspected type is returned.
        typedef typename network_boost::mpl::if_c<
                    network_boost::is_same<T, ::network_boostphoenix::nil_t>::value,
                    ::network_boostphoenix::nil_t,
                    ptr_type
                >::type
            type;
    };
    ...
    struct assign_zero_to_tuple_member {
        ...
        static void do_(TupleT &t) { t[::network_boostphoenix::tuple_index<N>()] = 0; }
    };

    template <int N>
    struct assign_zero_to_tuple_member<N, ::network_boostphoenix::nil_t> {
    ...
    struct phoenix_nil_type {
        ...
        typedef ::network_boostphoenix::nil_t type;
    };
    ...
    struct init_tuple_member {
        ...
        do_(TupleT &t)
        {
            typedef typename network_boost::mpl::eval_if_c<
                        (N < TupleT::length),
                        ::network_boostphoenix::tuple_element<N, TupleT>,
                        phoenix_nil_type
                    >::type
                element_type;
...
protected:
    ...
    //      {
    //          using ::network_boostphoenix::tuple_index_names::_1;
    //          t[_1] = &t0;
    //          ...
    //      }
    //
    ...
    #define BOOST_SPIRIT_GRAMMARDEF_ENUM_ASSIGN(z, N, _) \
        using ::network_boostphoenix::tuple_index_names::BOOST_PP_CAT(_, BOOST_PP_INC(N)); \
...
public:
    ...
    template <int N>
    typename ::network_boostphoenix::tuple_element<N, tuple_t>::crtype
    ...
    //  number of parameters used while calling start_parsers().
        BOOST_SPIRIT_ASSERT(0 != t[::network_boostphoenix::tuple_index<N>()]);

        return t[::network_boostphoenix::tuple_index<N>()];
    }
