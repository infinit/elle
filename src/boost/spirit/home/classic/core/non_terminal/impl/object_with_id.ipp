...
BOOST_SPIRIT_CLASSIC_NAMESPACE_BEGIN
    ...
    namespace impl {
        ...
        private:
#ifdef BOOST_SPIRIT_THREADSAFE
            static network_boost::mutex &mutex_instance();
            static void mutex_init();
#endif

            network_boost::shared_ptr<object_with_id_base_supply<IdT> > id_supply;
        };
        ...
        {
#ifdef BOOST_SPIRIT_THREADSAFE
            network_boost::unique_lock<network_boost::mutex> lock(mutex);
#endif
            if (free_ids.size())
            {
        ...
        {
            {
                ...
                static network_boost::once_flag been_here = BOOST_ONCE_INIT;
#else
                static network_boost::once_flag been_here;
#endif
                network_boost::call_once(been_here, mutex_init);
                network_boost::mutex &mutex = mutex_instance();
                network_boost::unique_lock<network_boost::mutex> lock(mutex);
#endif
                static network_boost::shared_ptr<object_with_id_base_supply<IdT> >
                    static_supply;
...
#ifdef BOOST_SPIRIT_THREADSAFE
        template <typename TagT, typename IdT>
        inline network_boost::mutex &
        object_with_id_base<TagT, IdT>::mutex_instance()
        {
            static network_boost::mutex mutex;
            return mutex;
        }
#endif
