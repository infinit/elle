...
namespace network_boost { namespace spirit { namespace qi
{
    ...
    template <typename Exposed, typename Transformed>
    struct transform_attribute<network_boost::optional<Exposed>, Transformed
      , typename disable_if<is_same<network_boost::optional<Exposed>, Transformed> >::type>
    {
        typedef Transformed& type;
        static Transformed& pre(network_boost::optional<Exposed>& val)
        {
            if (!val)
                val = Transformed();
            return network_boost::get<Transformed>(val);
        }
        static void post(network_boost::optional<Exposed>&, Transformed const&) {}
        static void fail(network_boost::optional<Exposed>& val)
        {
