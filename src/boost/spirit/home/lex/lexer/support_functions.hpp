...
namespace network_boost { namespace spirit { namespace lex
{
    ...
    inline typename expression::less<
        typename network_boostphoenix::as_actor<T>::type
    >::type const
    less(T const& v)
    {
        return expression::less<T>::make(network_boostphoenix::as_actor<T>::convert(v));
    }
    ...
    inline network_boostphoenix::actor<more_type> more()
    {
        return network_boostphoenix::actor<more_type>();
    }
    ...
    inline typename expression::lookahead<
        typename network_boostphoenix::as_actor<T>::type
      , typename network_boostphoenix::as_actor<std::size_t>::type
    >::type const
    lookahead(T const& id)
    {
        typedef typename network_boostphoenix::as_actor<T>::type id_actor_type;
        typedef typename network_boostphoenix::as_actor<std::size_t>::type state_actor_type;

        return expression::lookahead<id_actor_type, state_actor_type>::make(
            network_boostphoenix::as_actor<T>::convert(id),
            network_boostphoenix::as_actor<std::size_t>::convert(std::size_t(~0)));
    }

    template <typename Attribute, typename Char, typename Idtype>
    inline typename expression::lookahead<
        typename network_boostphoenix::as_actor<Idtype>::type
      , typename network_boostphoenix::as_actor<std::size_t>::type
    >::type const
    lookahead(token_def<Attribute, Char, Idtype> const& tok)
    {
        typedef typename network_boostphoenix::as_actor<Idtype>::type id_actor_type;
        typedef typename network_boostphoenix::as_actor<std::size_t>::type state_actor_type;
        ...
        return expression::lookahead<id_actor_type, state_actor_type>::make(
            network_boostphoenix::as_actor<Idtype>::convert(tok.id()),
            network_boostphoenix::as_actor<std::size_t>::convert(state));
    }
