...
namespace network_boost { namespace spirit { namespace lex { namespace lexertl
{
    ...
    namespace detail
    {
        ...
        //
        ///////////////////////////////////////////////////////////////////////
        inline network_boost::lexer::regex_flags map_flags(unsigned int flags)
        {
            unsigned int retval = network_boost::lexer::none;
            if (flags & match_flags::match_not_dot_newline)
                retval |= network_boost::lexer::dot_not_newline;
            if (flags & match_flags::match_icase)
                retval |= network_boost::lexer::icase;

            return network_boost::lexer::regex_flags(retval);
        }
    }
    ...
    public:
        ...
        typedef typename network_boost::detail::iterator_traits<Iterator>::value_type
            char_type;
        typedef std::basic_string<char_type> string_type;

        typedef network_boost::lexer::basic_rules<char_type> basic_rules_type;
    ...
    private:
        ...
        struct iterator_data_type
        {
            ...
            iterator_data_type(
                    network_boost::lexer::basic_state_machine<char_type> const& sm
                  , network_boost::lexer::basic_rules<char_type> const& rules
                  , semantic_actions_type const& actions)
              : state_machine_(sm), rules_(rules), actions_(actions)
            {}

            network_boost::lexer::basic_state_machine<char_type> const& state_machine_;
            network_boost::lexer::basic_rules<char_type> const& rules_;
    ...
    public:
        ...
        network_boost::lexer::rules const& get_rules() const { return rules_; }

        void clear(char_type const* state)
        {
            std::size_t s = rules_.state(state);
            if (network_boost::lexer::npos != s)
                rules_.clear(state);
            initialized_dfa_ = false;
        }
        std::size_t add_state(char_type const* state)
        {
            ...
            if (network_boost::lexer::npos == stateid) {
...
//         }
        ...
        bool init_dfa(bool minimize = false) const
        {
            if (!initialized_dfa_) {
                ...
                typedef network_boost::lexer::basic_generator<char_type> generator;
                generator::build (rules_, state_machine_);
                if (minimize)
                    generator::minimise (state_machine_);

#if defined(BOOST_SPIRIT_LEXERTL_DEBUG)
                network_boost::lexer::debug::dump(state_machine_, std::cerr);
#endif
...
//                 std::swap(rules, rules_);
    ...
    private:
        ...
        mutable network_boost::lexer::basic_state_machine<char_type> state_machine_;
        network_boost::lexer::regex_flags flags_;
