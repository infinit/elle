...
{ 
    ...
    struct token<Iterator, lex::omit, mpl::false_, Idtype>
    {
        ...
        token() : id_(id_type(network_boost::lexer::npos)) {}
        ...
        bool is_valid() const 
        { 
            return 0 != id_ && id_type(network_boost::lexer::npos) != id_; 
        }
    ...
    public:
        ...
        //  default constructed tokens correspond to EOI tokens
        token() : state_(network_boost::lexer::npos) {}

        //  construct an invalid token
        explicit token(int) : base_type(0), state_(network_boost::lexer::npos) {}

        token(id_type id, std::size_t state)
          : base_type(id, network_boost::lexer::npos), state_(state) {}

        token(id_type id, std::size_t state, token_value_type)
          : base_type(id, network_boost::lexer::npos, unused)
          , state_(state) {}

        token(id_type id, std::size_t state
              , Iterator const& first, Iterator const& last)
          : base_type(id, network_boost::lexer::npos, first, last)
          , state_(state) {}
      ...
      , lex::lexertl::token<Iterator, AttributeTypes, HasState, Idtype> >
    {
        ...
        //  converted value to be returned from subsequent calls.
            ...
            //  first access to the token value
                typedef iterator_range<Iterator> iterpair_type;
                iterpair_type const& ip = network_boost::get<iterpair_type>(t.value());
            ...
            // reuse the already assigned value
                spirit::traits::assign_to(network_boost::get<Attribute>(t.value()), attr);
            }
        }
    };
