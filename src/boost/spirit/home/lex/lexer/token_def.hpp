...
namespace network_boost { namespace spirit { namespace lex
{
    ...
    public:
        ...
        {
            if (0 == def_.which()) 
                return info("token_def", network_boost::get<string_type>(def_));

            return info("token_def", network_boost::get<char_type>(def_));
        }
          ...
          , String const& targetstate) const
        {
            ...
            if (0 == def_.which()) {
                unique_id_ = lexdef.add_token(state.c_str()
                  , network_boost::get<string_type>(def_), token_id_, target);
            }
            else {
                unique_id_ = lexdef.add_token(state.c_str()
                  , network_boost::get<char_type>(def_), token_id_, target);
            }
        }
    ...
    public:
        ...
        { 
            return (0 == def_.which()) ? 
                network_boost::get<string_type>(def_) : 
                string_type(1, network_boost::get<char_type>(def_));
        }
