...
namespace network_boost { namespace spirit { namespace traits
{
    ...
    struct extract_from_attribute<network_boost::optional<Attribute>, Exposed>
    {
        typedef Attribute const& type;

        template <typename Context>
        static type call(network_boost::optional<Attribute> const& attr, Context& ctx)
        {
            return extract_from<Exposed>(network_boost::get<Attribute>(attr), ctx);
        }
    };

    template <typename Attribute, typename Exposed>
    struct extract_from_attribute<network_boost::optional<Attribute const>, Exposed>
    {
        typedef Attribute const& type;

        template <typename Context>
        static type call(network_boost::optional<Attribute const> const& attr, Context& ctx)
        {
            return extract_from<Exposed>(network_boost::get<Attribute const>(attr), ctx);
        }
    };

    // This handles attributes wrapped inside a network_boost::ref().
    template <typename Attribute, typename Exposed>
