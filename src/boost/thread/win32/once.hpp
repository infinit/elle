...
#define BOOST_THREAD_INVOKE_RET_VOID_CALL
#else
#define BOOST_THREAD_INVOKE_RET_VOID network_boost::bind
#define BOOST_THREAD_INVOKE_RET_VOID_CALL ()
#endif
    ...
    namespace detail
    {
...
#ifdef BOOST_NO_ANSI_APIS
            return ::network_boost::detail::win32::OpenEventW(
#else
            return ::network_boost::detail::win32::OpenEventA(
#endif
                ::network_boost::detail::win32::synchronize |
                ::network_boost::detail::win32::event_modify_state,
                false,
                mutex_name);
        }
        ...
        inline void* create_once_event(once_char_type* mutex_name,void* flag_address)
        {
            ...
            return ::network_boost::detail::win32::create_event(
                mutex_name, 
                ::network_boost::detail::win32::manual_reset_event,
                ::network_boost::detail::win32::event_initially_reset);
        }
        ...
        inline bool enter_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
          ...
          if(!status)
          {
            ...
            if(ctx.event_handle)
            {
                ::network_boost::detail::win32::ResetEvent(ctx.event_handle);
            }
            return true;
          }
          return false;
        ...
        inline void commit_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
          ...
          if(!ctx.event_handle &&
             (::network_boost::detail::interlocked_read_acquire(&flag.count)>1))
          {
              ctx.event_handle=detail::create_once_event(ctx.mutex_name,&flag);
          }
          if(ctx.event_handle)
          {
              ::network_boost::detail::win32::SetEvent(ctx.event_handle);
          }
        }
        inline void rollback_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
...
//#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR)
    inline void call_once(once_flag& flag, void (*f)())
    {
        ...
        while(::network_boost::detail::interlocked_read_acquire(&flag.status)
              !=ctx.function_complete_flag_value)
        {
            ...
            if(!ctx.counted)
            {
                BOOST_INTERLOCKED_INCREMENT(&flag.count);
                ctx.counted=true;
                long status=::network_boost::detail::interlocked_read_acquire(&flag.status);
            ...
            BOOST_VERIFY(!::network_boost::detail::win32::WaitForSingleObjectEx(
                             ctx.event_handle,::network_boost::detail::win32::infinite, 0));
        }
    }
//#endif
            ...
            if(detail::enter_once_region(flag, ctx))
            {
                BOOST_TRY
                {
                  BOOST_THREAD_INVOKE_RET_VOID(
                        thread_detail::decay_copy(network_boost::forward<Function>(f)),
                        thread_detail::decay_copy(network_boost::forward<A>(a)),
        ...
        void call_once(once_flag& flag,void (*f)(BOOST_THREAD_RV_REF(T1)), BOOST_THREAD_RV_REF(T1) p1)
        {
                  ...
                  !=ctx.function_complete_flag_value)
            {
                if(detail::enter_once_region(flag, ctx))
                {
                    ...
                    {
                       f(
                           thread_detail::decay_copy(network_boost::forward<T1>(p1))
                       );
                    }
                    BOOST_CATCH(...)
                    {
                  ...
                  !=ctx.function_complete_flag_value)
            {
                if(detail::enter_once_region(flag, ctx))
                {
                    BOOST_TRY
                    {
                      f(
                          thread_detail::decay_copy(network_boost::forward<T1>(p1)),
                          thread_detail::decay_copy(network_boost::forward<T2>(p2))
                      );
                    }
                    BOOST_CATCH(...)
                    {
                ...
                if(detail::enter_once_region(flag, ctx))
                {
                    BOOST_TRY
                    {
                      f(
                          thread_detail::decay_copy(network_boost::forward<T1>(p1)),
                          thread_detail::decay_copy(network_boost::forward<T2>(p2)),
                          thread_detail::decay_copy(network_boost::forward<T3>(p3))
                      );
                    }
                    BOOST_CATCH(...)
                    {
