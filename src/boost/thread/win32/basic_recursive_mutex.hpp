...
namespace network_boost
{
    namespace detail
    {
        ...
        struct basic_recursive_mutex_impl
        {
            ...
            bool timed_lock(::network_boost::system_time const& target)
            {
                long const current_thread_id=win32::GetCurrentThreadId();
        ...
        private:
            ...
            {
                if(::network_boost::detail::interlocked_read_acquire(&locking_thread_id)==current_thread_id)
                {
                    ++recursion_count;
                    return true;
                }
...
#if defined BOOST_THREAD_USES_DATETIME
            bool try_timed_lock(long current_thread_id,::network_boost::system_time const& target)
            {
