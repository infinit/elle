...
namespace network_boost
{
    ...
    public:
        ...
        void lock_shared()
        {
            ...
            BOOST_VERIFY(timed_lock_shared(::network_boost::detail::get_system_time_sentinel()));
#else
        ...
        bool timed_lock_shared(network_boost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
                for(;;)
                {
                    ...
                    if(new_state.exclusive || new_state.exclusive_waiting_blocked)
                    {
                        ...
                        {
                            network_boost::throw_exception(network_boost::lock_error());
                        }
                    }
                    else
                    {
                ...
                unsigned long const res=detail::win32::WaitForSingleObjectEx(semaphores[unlock_sem],::network_boost::detail::get_milliseconds_until(wait_until), 0);
...
#if defined BOOST_THREAD_USES_DATETIME
            BOOST_VERIFY(timed_lock(::network_boost::detail::get_system_time_sentinel()));
#else
...
#if defined BOOST_THREAD_USES_DATETIME
        bool timed_lock(network_boost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
                ...
                #endif
                unsigned long const wait_res=detail::win32::WaitForMultipleObjectsEx(2,semaphores,wait_all,::network_boost::detail::get_milliseconds_until(wait_until), 0);
