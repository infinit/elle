...
// network_boost::thread::future requires exception handling
// due to network_boost::exception::exception_ptr dependency
...
namespace network_boost
{
  ...
  shared_ptr<T const> static_shared_from_this(T const* that)
  {
    ...
    namespace detail
    {
        ...
        struct relocker
        {
            network_boost::unique_lock<network_boost::mutex>& lock_;

            relocker(network_boost::unique_lock<network_boost::mutex>& lk):
                lock_(lk)
            {
                lock_.unlock();
            }
        ...
        {
            typedef std::list<network_boost::condition_variable_any*> waiter_list;
            ...
            network_boost::exception_ptr exception;
            bool done;
            bool is_valid_;
            bool is_deferred_;
            bool is_constructed;
            launch policy_;
            mutable network_boost::mutex mutex;
            network_boost::condition_variable waiters;
            waiter_list external_waiters;
            network_boost::function<void()> callback;
            ...
            void set_executor_policy(executor_ptr_type aex, network_boost::lock_guard<network_boost::mutex>&)
            {
              set_executor();
              ex = aex;
            }
            void set_executor_policy(executor_ptr_type aex, network_boost::unique_lock<network_boost::mutex>&)
            {
              set_executor();
              ex = aex;
            }

            bool valid(network_boost::unique_lock<network_boost::mutex>&) { return is_valid_; }
            bool valid() {
              network_boost::unique_lock<network_boost::mutex> lk(this->mutex);
              return valid(lk);
            }
            void invalidate(network_boost::unique_lock<network_boost::mutex>&) { is_valid_ = false; }
            void invalidate() {
              network_boost::unique_lock<network_boost::mutex> lk(this->mutex);
              invalidate(lk);
            }
            void validate(network_boost::unique_lock<network_boost::mutex>&) { is_valid_ = true; }
            void validate() {
              network_boost::unique_lock<network_boost::mutex> lk(this->mutex);
              validate(lk);
            }
...
#endif
            notify_when_ready_handle notify_when_ready(network_boost::condition_variable_any& cv)
            {
                network_boost::unique_lock<network_boost::mutex> lock(this->mutex);
                do_callback(lock);
            ...
            {
                network_boost::lock_guard<network_boost::mutex> lock(this->mutex);
                external_waiters.erase(it);
            }

#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            void do_continuation(network_boost::unique_lock<network_boost::mutex>& lock)
            {
...
#else
            void do_continuation(network_boost::unique_lock<network_boost::mutex>&)
            {
            }
#endif
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            virtual void set_continuation_ptr(continuation_ptr_type continuation, network_boost::unique_lock<network_boost::mutex>& lock)
            {
...
#endif
            void mark_finished_internal(network_boost::unique_lock<network_boost::mutex>& lock)
            {
                done=true;
                waiters.notify_all();
            ...
            void do_callback(network_boost::unique_lock<network_boost::mutex>& lock)
            {
                if(callback && !done)
                {
                    network_boost::function<void()> local_callback=callback;
            ...
            void wait_internal(network_boost::unique_lock<network_boost::mutex> &lk, bool rethrow=true)
            {
              do_callback(lk);
              if (is_deferred_)
              {
              ...
              {
                  network_boost::rethrow_exception(exception);
              }
            }

            virtual void wait(network_boost::unique_lock<network_boost::mutex>& lock, bool rethrow=true)
            {
            ...
            bool timed_wait_until(network_boost::system_time const& target_time)
            {
                network_boost::unique_lock<network_boost::mutex> lock(this->mutex);
                if (is_deferred_)
                    return false;
...
#endif
            void mark_exceptional_finish_internal(network_boost::exception_ptr const& e, network_boost::unique_lock<network_boost::mutex>& lock)
            {
                exception=e;
                mark_finished_internal(lock);
            }
            ...
            {
                network_boost::unique_lock<network_boost::mutex> lock(this->mutex);
                mark_exceptional_finish_internal(network_boost::current_exception(), lock);
            }

            void set_exception_at_thread_exit(exception_ptr e)
            {
              unique_lock<network_boost::mutex> lk(this->mutex);
              if (has_value(lk))
              {
            ...
            bool has_value(unique_lock<network_boost::mutex>& )  const
            {
                return done && ! exception;
            }
            ...
            launch launch_policy(network_boost::unique_lock<network_boost::mutex>&) const
            {
                return policy_;
            }

            future_state::state get_state(network_boost::unique_lock<network_boost::mutex>&) const
            {
                if(!done)
                {
            ...
            {
                network_boost::lock_guard<network_boost::mutex> guard(this->mutex);
                if(!done)
                {
                    return future_state::waiting;
                }
            ...
            {
                network_boost::lock_guard<network_boost::mutex> lock(this->mutex);
                callback=network_boost::bind(f,network_boost::ref(*u));
            }

            virtual void execute(network_boost::unique_lock<network_boost::mutex>&) {}
        ...
        {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
              typedef network_boost::optional<T> storage_type;
#else
              typedef network_boost::csbl::unique_ptr<T> storage_type;
#endif
            ...
            typedef typename conditional<network_boost::is_fundamental<T>::value,T,T const&>::type source_reference_type;
            typedef BOOST_THREAD_RV_REF(T) rvalue_source_type;
            typedef T move_dest_type;
#else
            typedef T& source_reference_type;
            typedef typename conditional<network_boost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >::value, BOOST_THREAD_RV_REF(T),T const&>::type rvalue_source_type;
            typedef typename conditional<network_boost::thread_detail::is_convertible<T&,BOOST_THREAD_RV_REF(T) >::value, BOOST_THREAD_RV_REF(T),T>::type move_dest_type;
#endif
            ...
            void mark_finished_with_result_internal(source_reference_type result_, network_boost::unique_lock<network_boost::mutex>& lock)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result = result_;
#else
            ...
            void mark_finished_with_result_internal(rvalue_source_type result_, network_boost::unique_lock<network_boost::mutex>& lock)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result = network_boost::move(result_);
#elif ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                result.reset(new T(network_boost::move(result_)));
#else
            ...
            template <class ...Args>
            void mark_finished_with_result_internal(network_boost::unique_lock<network_boost::mutex>& lock, BOOST_THREAD_FWD_REF(Args)... args)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result.emplace(network_boost::forward<Args>(args)...);
#else
                result.reset(new T(network_boost::forward<Args>(args)...));
#endif
                this->mark_finished_internal(lock);
            }
#endif
            ...
            void mark_finished_with_result(rvalue_source_type result_)
            {
                ...
                mark_finished_with_result_internal(network_boost::move(result_), lock);
#else
                mark_finished_with_result_internal(static_cast<rvalue_source_type>(result_), lock);
#endif
            }

            storage_type& get_storage(network_boost::unique_lock<network_boost::mutex>& lk)
            {
                wait_internal(lk);
                return result;
            }
            virtual move_dest_type get(network_boost::unique_lock<network_boost::mutex>& lk)
            {
                return network_boost::move(*get_storage(lk));
            }
            move_dest_type get()
            {
                network_boost::unique_lock<network_boost::mutex> lk(this->mutex);
            ...
            virtual shared_future_get_result_type get_sh(network_boost::unique_lock<network_boost::mutex>& lk)
            {
            ...
            detail::shared_state_base
        {
            ...
            virtual T& get(network_boost::unique_lock<network_boost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
            ...
            virtual T& get_sh(network_boost::unique_lock<network_boost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
            ...
            detail::shared_state_base
        {
            ...
            void mark_finished_with_result_internal(network_boost::unique_lock<network_boost::mutex>& lock)
            {
                mark_finished_internal(lock);
            }
            ...
            virtual void get(network_boost::unique_lock<network_boost::mutex>& lock)
            {
            ...
            virtual void get_sh(network_boost::unique_lock<network_boost::mutex>& lock)
            {
                this->wait_internal(lock);
            }
            void get_sh()
        ...
        public:
          ...
          virtual void wait(network_boost::unique_lock<network_boost::mutex>& lk, bool rethrow)
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
              {
                relocker rlk(lk);
        ...
        struct future_async_shared_state: future_async_shared_state_base<Rp>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = thread(&future_async_shared_state::run, static_shared_from_this(this), network_boost::forward<Fp>(f));
#else
            thread(&future_async_shared_state::run, static_shared_from_this(this), network_boost::forward<Fp>(f)).detach();
#endif
          }
        ...
        struct future_async_shared_state<void, Fp>: public future_async_shared_state_base<void>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = thread(&future_async_shared_state::run, static_shared_from_this(this), network_boost::move(f));
#else
            thread(&future_async_shared_state::run, static_shared_from_this(this), network_boost::move(f)).detach();
#endif
          }
        ...
        public:
          ...
          : func_(network_boost::move(f))
          {
            this->set_deferred();
          }

          virtual void execute(network_boost::unique_lock<network_boost::mutex>& lck) {
            try
            {
              Fp local_fuct=network_boost::move(func_);
              relocker relock(lck);
              Rp res = local_fuct();
              relock.lock();
              this->mark_finished_with_result_internal(network_boost::move(res), lck);
            }
            catch (...)
            {
        ...
        private:
            ...
            struct registered_waiter
            {
                network_boost::shared_ptr<detail::shared_state_base> future_;
                detail::shared_state_base::notify_when_ready_handle handle;
                count_type index;

                registered_waiter(network_boost::shared_ptr<detail::shared_state_base> const& a_future,
            ...
            struct all_futures_lock
            {
                   ...
                   network_boost::scoped_array<network_boost::unique_lock<network_boost::mutex> > locks;

                all_futures_lock(std::vector<registered_waiter>& futures):
                    count(futures.size()),locks(new network_boost::unique_lock<network_boost::mutex>[count])
                {
                    for(count_type_portable i=0;i<count;++i)
                    {
                        locks[i]=BOOST_THREAD_MAKE_RV_REF(network_boost::unique_lock<network_boost::mutex>(futures[i].future_->mutex));
                    }
                }

                void lock()
                {
                    network_boost::lock(locks.get(),locks.get()+count);
                }
            ...
            network_boost::condition_variable_any cv;
...
//    typename network_boost::disable_if<is_future_type<Iterator>,Iterator>::type wait_for_any(Iterator begin,Iterator end)
//    {
//        if(begin==end)
//            return end;
//
...
//            waiter.add(*current);
//        }
//        return network_boost::next(begin,waiter.wait());
//    }

#ifdef BOOST_NO_CXX11_VARIADIC_TEMPLATES
    template<typename F1,typename F2>
    typename network_boost::enable_if<is_future_type<F1>,typename detail::future_waiter::count_type>::type wait_for_any(F1& f1,F2& f2)
    {
        detail::future_waiter waiter;
        waiter.add(f1);
        waiter.add(f2);
...
#else
    template<typename F1, typename... Fs>
    typename network_boost::enable_if<is_future_type<F1>, typename detail::future_waiter::count_type>::type
    ...
    namespace detail
    {
      ...
      public:
        ...
        typedef network_boost::shared_ptr<detail::shared_state<R> > future_ptr;
      ...
      public:
        ...
        state get_state(network_boost::unique_lock<network_boost::mutex>& lk) const
        {
            if(!future_)
            {
        ...
        bool is_ready(network_boost::unique_lock<network_boost::mutex>& lk) const
        {
        ...
        launch launch_policy(network_boost::unique_lock<network_boost::mutex>& lk) const
        {
        ...
        {
          if ( future_ ) {
            network_boost::unique_lock<network_boost::mutex> lk(this->future_->mutex);
        ...
        void wait() const
        {
            if(!future_)
            {
                network_boost::throw_exception(future_uninitialized());
            }
            future_->wait(false);
        }
        ...
        {
            return timed_wait_until(network_boost::get_system_time()+rel_time);
        }

        bool timed_wait_until(network_boost::system_time const& abs_time) const
        {
            if(!future_)
            {
                network_boost::throw_exception(future_uninitialized());
    ...
    namespace detail
    {
        ...
        BOOST_THREAD_FUTURE<Rp>
        make_future_async_continuation_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_deferred_continuation_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_deferred_continuation_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_async_continuation_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_executor_continuation_shared_state(Ex& ex, network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_executor_continuation_shared_state(Ex& ex, network_boost::unique_lock<network_boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        make_future_unwrap_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
    }
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      typename network_boost::disable_if<is_future_type<InputIterator>,
    ...
    private:
        ...
        detail::make_future_async_continuation_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_deferred_continuation_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_deferred_continuation_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_async_continuation_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_executor_continuation_shared_state(Ex& ex, network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_executor_continuation_shared_state(Ex& ex, network_boost::unique_lock<network_boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        template <class F, class Rp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_unwrap_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      friend typename network_boost::disable_if<is_future_type<InputIterator>,
        BOOST_THREAD_FUTURE<csbl::vector<typename InputIterator::value_type>  >
      >::type
      when_all(InputIterator first, InputIterator last);
    ...
    public:
        ...
        base_type(network_boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))))
        {
        }
        ...
        {
            this->base_type::operator=(network_boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }

        shared_future<R> share()
        {
          return shared_future<R>(::network_boost::move(*this));
        }
        ...
        move_dest_type get()
        {
            ...
            unique_lock<network_boost::mutex> lk(this->future_->mutex);
        ...
        typename network_boost::disable_if< is_void<R2>, move_dest_type>::type
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename network_boost::result_of<F(BOOST_THREAD_FUTURE)>::type>
        ...
        template <typename R2>
        inline typename network_boost::disable_if< is_void<R2>, BOOST_THREAD_FUTURE<R> >::type
      ...
      when_any(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures);
        ...
        public:
            ...
            move_dest_type get_or(BOOST_THREAD_RV_REF(R) v) // EXTENSION
            {
                ...
                else return network_boost::move(v);
            }
    ...
    public:
        ...
        {
            base_type::operator=(network_boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }
        ...
        template <typename R2>
        typename network_boost::disable_if< is_void<R2>, typename detail::shared_state<R>::shared_future_get_result_type>::type
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename network_boost::result_of<F(shared_future)>::type>
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    public:
        ...
        promise(network_boost::allocator_arg_t, Allocator a)
        {
        ...
        ~promise()
        {
            if(future_)
            {
                network_boost::unique_lock<network_boost::mutex> lock(future_->mutex);

                if(!future_->done && !future_->is_constructed)
                {
                    future_->mark_exceptional_finish_internal(network_boost::copy_exception(broken_promise()), lock);
                }
            }
        }
        ...
        void set_executor(executor_ptr_type aex)
        {
          ...
          if (future_.get()==0)
          {
              network_boost::throw_exception(promise_moved());
          }
          network_boost::lock_guard<network_boost::mutex> lk(future_->mutex);
        ...
        BOOST_THREAD_FUTURE<R> get_future()
        {
            ...
            if (future_obtained)
            {
                network_boost::throw_exception(future_already_retrieved());
            }
...
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename network_boost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type set_value(TR const &  r)
        {
            lazy_init();
            network_boost::unique_lock<network_boost::mutex> lock(future_->mutex);
            if(future_->done)
            {
                network_boost::throw_exception(promise_already_satisfied());
            }
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            future_->mark_finished_with_result_internal(network_boost::move(r), lock);
#else
        ...
        void emplace(BOOST_THREAD_FWD_REF(Args) ...args)
        {
            ...
            future_->mark_finished_with_result_internal(lock, network_boost::forward<Args>(args)...);
        }

#endif

        void set_exception(network_boost::exception_ptr p)
        {
            lazy_init();
            network_boost::unique_lock<network_boost::mutex> lock(future_->mutex);
            if(future_->done)
        ...
        {
          set_exception(network_boost::copy_exception(ex));
        }
        // setting the result with deferred notification
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename network_boost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type set_value_at_thread_exit(TR const& r)
        {
          if (future_.get()==0)
          {
              network_boost::throw_exception(promise_moved());
        ...
        void set_value_at_thread_exit(BOOST_THREAD_RV_REF(R) r)
        {
          ...
          future_->set_value_at_thread_exit(network_boost::move(r));
        }
        void set_exception_at_thread_exit(exception_ptr e)
        {
          if (future_.get()==0)
        ...
        {
          set_exception_at_thread_exit(network_boost::copy_exception(ex));
        }
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    class promise<void>
    {
        typedef network_boost::shared_ptr<detail::shared_state<void> > future_ptr;
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CTOR_ALLOCATORS
namespace network_boost { namespace container {
    template <class R, class Alloc>
    struct uses_allocator< ::network_boost::promise<R> , Alloc> : true_type
    {
    };
}}
#if ! defined  BOOST_NO_CXX11_ALLOCATOR
...
namespace network_boost
{
    ...
    namespace detail
    {
      ...
      struct task_base_shared_state:
            ...
            detail::shared_state<R>
        {
            ...
            {
                {
                    network_boost::lock_guard<network_boost::mutex> lk(this->mutex);
                    if(started)
                    {
                        network_boost::throw_exception(task_already_started());
                    }
                    started=true;
                }
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_run(network_boost::move(args)...);
#else
                do_run();
#endif
            }
...
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_apply(network_boost::move(args)...);
#else
                do_apply();
#endif
            }
            ...
            void owner_destroyed()
            {
                ...
                {
                    started=true;
                    this->mark_exceptional_finish_internal(network_boost::copy_exception(network_boost::broken_promise()), lk);
                }
            }
        };
        ...
        public:
            ...
            task_shared_state(BOOST_THREAD_RV_REF(F) f_):
              f(network_boost::move(f_))
            {}

            F callable()
            {
              return network_boost::move(f);
            }
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->set_value_at_thread_exit(f(network_boost::move(args)...));
                }
#else
            void do_apply()
            {
            ...
            void do_run(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->mark_finished_with_result(f(network_boost::move(args)...));
                }
#else
            void do_run()
            {
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                  R res((f()));
                  this->mark_finished_with_result(network_boost::move(res));
#else
           ...
           task_base_shared_state<R>
            ...
            public:
                ...
                void do_apply()
                {
                    ...
                    {
                        R r((f()));
                        this->set_value_at_thread_exit(network_boost::move(r));
                    }
#endif
                    catch(...)
                    {
    ...
    {
      typedef network_boost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task_ptr;
      network_boost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task;
  #else
    template<typename R>
    class packaged_task<R()>
    {
      typedef network_boost::shared_ptr<detail::task_base_shared_state<R()> > task_ptr;
      network_boost::shared_ptr<detail::task_base_shared_state<R()> > task;
  #endif
#else
    template<typename R>
    class packaged_task
    ...
    public:
        ...
        explicit packaged_task(R(*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {
            ...
            task= task_ptr(new task_shared_state_type(f, network_boost::move(args)...));
            future_obtained=false;
        }
  #else
        ...
        explicit packaged_task(BOOST_THREAD_FWD_REF(F) f
            , typename network_boost::disable_if<is_same<typename decay<F>::type, packaged_task>, dummy* >::type=0
            )
        {
          typedef typename decay<F>::type FR;
            ...
            typedef detail::task_shared_state<FR,R> task_shared_state_type;
#endif
            task = task_ptr(new task_shared_state_type(network_boost::forward<F>(f)));
            future_obtained = false;
...
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
            typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(network_boost::move(f)));
#else
            typedef detail::task_shared_state<F,R()> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(network_boost::move(f)));
#endif
...
#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR)
        template <class Allocator>
        packaged_task(network_boost::allocator_arg_t, Allocator a, R(*f)())
        {
          typedef R(*FR)();
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
        ...
        packaged_task(network_boost::allocator_arg_t, Allocator a, BOOST_THREAD_FWD_REF(F) f)
        {
          typedef typename decay<F>::type FR;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(network_boost::forward<F>(f)), D(a2, 1) );
          future_obtained = false;
        }
#else // ! defined BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class F, class Allocator>
        packaged_task(network_boost::allocator_arg_t, Allocator a, const F& f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
        ...
        template <class F, class Allocator>
        packaged_task(network_boost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(network_boost::move(f)), D(a2, 1) );
          future_obtained = false;
        }
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            packaged_task temp(network_boost::move(other));
#else
        ...
        {
          if (!valid())
              network_boost::throw_exception(task_moved());
          network_boost::lock_guard<network_boost::mutex> lk(task->mutex);
          task->set_executor_policy(aex, lk);
        }
#endif
        void reset() {
            if (!valid())
              network_boost::throw_exception(future_error(system::make_error_code(future_errc::no_state)));
        ...
        void operator()(ArgTypes... args) {
            ...
            task->run(network_boost::move(args)...);
        }
        void make_ready_at_thread_exit(ArgTypes... args) {
          ...
          task->apply(network_boost::move(args)...);
        }
#else
        void operator()() {
            if(!task) {
        ...
        void make_ready_at_thread_exit() {
          ...
          if (task->has_value()) network_boost::throw_exception(promise_already_satisfied());
          task->apply();
        }
#endif
        template<typename F>
...
namespace network_boost { namespace container {
    template <class R, class Alloc>
    struct uses_allocator< ::network_boost::packaged_task<R> , Alloc> : true_type
    {};
}}
#if ! defined  BOOST_NO_CXX11_ALLOCATOR
namespace std {
...
namespace detail
{
  ...
  make_future_deferred_shared_state(BOOST_THREAD_FWD_REF(Fp) f) {
    shared_ptr<future_deferred_shared_state<Rp, Fp> >
        h(new future_deferred_shared_state<Rp, Fp>(network_boost::forward<Fp>(f)));
  ...
  make_future_async_shared_state(BOOST_THREAD_FWD_REF(Fp) f) {
    ...
    h->init(network_boost::forward<Fp>(f));
    return BOOST_THREAD_FUTURE<Rp>(h);
  }
}
  ...
  async(launch policy, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF(network_boost::detail::make_future_async_shared_state<Rp>(
              BF(
                  f
                  , thread_detail::decay_copy(network_boost::forward<ArgTypes>(args))...
              )
          ));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF(network_boost::detail::make_future_deferred_shared_state<Rp>(
              BF(
                  f
                  , thread_detail::decay_copy(network_boost::forward<ArgTypes>(args))...
              )
  ...
  async(launch policy, R(*f)()) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      ...
      ret.set_async();
      network_boost::thread( network_boost::move(pt) ).detach();
      return ::network_boost::move(ret);
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      std::terminate();
      //BOOST_THREAD_FUTURE<R> ret;
  ...
  template <class F, class ...ArgTypes>
  BOOST_THREAD_FUTURE<typename network_boost::result_of<typename decay<F>::type(
  ...
  async(launch policy, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF(network_boost::detail::make_future_async_shared_state<Rp>(
              BF(
                  thread_detail::decay_copy(network_boost::forward<F>(f))
                , thread_detail::decay_copy(network_boost::forward<ArgTypes>(args))...
              )
          ));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
  ...
  template <class F>
  BOOST_THREAD_FUTURE<typename network_boost::result_of<typename decay<F>::type()>::type>
  async(launch policy, BOOST_THREAD_FWD_REF(F) f) {
    typedef typename network_boost::result_of<typename decay<F>::type()>::type R;
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
    typedef packaged_task<R()> packaged_task_type;
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      packaged_task_type pt( network_boost::forward<F>(f) );
      ...
      //BOOST_THREAD_FUTURE<R> ret;
      //return ::network_boost::move(ret);
      //          return network_boost::detail::make_future_deferred_shared_state<Rp>(
      //              BF(
      //                  thread_detail::decay_copy(network_boost::forward<F>(f))
      //              )
      //          );
...
namespace detail {
    ...
    public:
      ...
      : that(st), f_(network_boost::move(f))
      {};
      ...
      : that(x.that), f_(network_boost::move(x.f_))
      {
        x.that.reset();
      }
...
namespace detail {
    ...
    public:
      ...
      void init(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f)
      {
        ...
        shared_state_nullary_task<Rp,Cont> t(this->shared_from_this(), network_boost::forward<Fp>(f));
        ex.submit(network_boost::move(t));
      }
    ...
    make_future_executor_shared_state(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f) {
      ...
      h->init(ex, network_boost::forward<Fp>(f));
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
...
//#if ! defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(network_boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(network_boost::forward<ArgTypes>(args))...
        )
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(A1)), BOOST_THREAD_FWD_REF(A1) a1) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(network_boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(network_boost::forward<A1>(a1))
        )
    ));
  }
#endif // defined BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f)  {
    ...
    return network_boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(network_boost::forward<F>(f))
        )
    );
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(network_boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(network_boost::forward<F>(f))
          , thread_detail::decay_copy(network_boost::forward<A1>(a1))
          , thread_detail::decay_copy(network_boost::forward<A2>(a2))
        )
    ));
  }
  ...
  async(R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), f, network_boost::forward<ArgTypes>(args)...));
  }
  #else
  template <class R>
  BOOST_THREAD_FUTURE<R>
  ...
  async(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args) {
      return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), network_boost::forward<F>(f), network_boost::forward<ArgTypes>(args)...));
  }
#else
  template <class F>
  BOOST_THREAD_FUTURE<typename network_boost::result_of<F()>::type>
  async(BOOST_THREAD_FWD_REF(F) f) {
      return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), network_boost::forward<F>(f)));
  }
#endif
  ...
  BOOST_THREAD_FUTURE<typename decay<T>::type> make_future(BOOST_THREAD_FWD_REF(T) value) {
    ...
    p.set_value(network_boost::forward<future_value_type>(value));
  ...
  BOOST_THREAD_FUTURE<typename detail::deduced_type<T>::type> make_ready_future(BOOST_THREAD_FWD_REF(T) value) {
    ...
    promise<future_value_type> p;
    p.set_value(network_boost::forward<T>(value));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future(E ex) {
    promise<T> p;
    p.set_exception(network_boost::copy_exception(ex));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future() {
    promise<T> p;
    p.set_exception(network_boost::current_exception());
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  template <typename T>
...
namespace detail
{
  ...
  public:
    ...
    : parent(network_boost::move(f)),
      continuation(network_boost::move(c))
    {
    }

    void init(network_boost::unique_lock<network_boost::mutex> &lock)
    {
    ...
    void call() {
      try {
        this->mark_finished_with_result(this->continuation(network_boost::move(this->parent)));
      } catch(...) {
        this->mark_exceptional_finish();
      }
    ...
    static void run(shared_ptr<network_boost::detail::shared_state_base> that_)
    {
  ...
  public:
    ...
    {
      try {
        this->continuation(network_boost::move(this->parent));
        this->mark_finished_with_result();
      } catch(...) {
        this->mark_exceptional_finish();
      }
  ...
  public:
    ...
    : base_type(network_boost::move(f), network_boost::forward<Fp>(c))
    {    }
...
namespace detail {
  ...
  public:
    ...
    template <class Ex>
    void init(network_boost::unique_lock<network_boost::mutex> &lk, Ex& ex)
    {
    ...
    void launch_continuation() {
      ...
      this->get_executor()->submit(network_boost::move(fct));
    }
  ...
  public:
    ...
    virtual void execute(network_boost::unique_lock<network_boost::mutex>& lk) {
      this->parent.wait();
      this->call(lk);
    }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_deferred_continuation_shared_state(
      network_boost::unique_lock<network_boost::mutex> &lock,
      BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new future_deferred_continuation_shared_state<F, Rp, Cont>(network_boost::move(f), network_boost::forward<Fp>(c)));
    h->init(lock);
    return BOOST_THREAD_FUTURE<Rp>(h);
  }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_async_continuation_shared_state(
      network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_async_continuation_shared_state<F,Rp, Cont> >
        h(new future_async_continuation_shared_state<F,Rp, Cont>(network_boost::move(f), network_boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<future_executor_continuation_shared_state<F,Rp, Cont> >
        h(new future_executor_continuation_shared_state<F,Rp, Cont>(network_boost::move(f), network_boost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  make_shared_future_deferred_continuation_shared_state(
    ...
    shared_ptr<shared_future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_deferred_continuation_shared_state<F, Rp, Cont>(f, network_boost::forward<Fp>(c)));
    h->init(lock);
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_shared_future_async_continuation_shared_state(
      network_boost::unique_lock<network_boost::mutex> &lock, F f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<shared_future_async_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_async_continuation_shared_state<F,Rp, Cont>(f, network_boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<shared_future_executor_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_executor_continuation_shared_state<F, Rp, Cont>(f, network_boost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename network_boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type>
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func) {
    typedef typename network_boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    ...
    network_boost::unique_lock<network_boost::mutex> lock(sentinel->mutex);

    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((network_boost::detail::make_future_async_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, network_boost::move(*this), network_boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((network_boost::detail::make_future_deferred_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, network_boost::move(*this), network_boost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
      ...
      return BOOST_THREAD_MAKE_RV_REF((network_boost::detail::make_future_executor_continuation_shared_state<Ex, BOOST_THREAD_FUTURE<R>, future_type>(ex,
                    lock, network_boost::move(*this), network_boost::forward<F>(func)
                )));
#endif
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_FWD_REF(F) func)  {
    ...
    return this->then(this->launch_policy(), network_boost::forward<F>(func));
#else
    typedef typename network_boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_!=0, future_uninitialized());
  ...
  template <typename R2>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename network_boost::result_of<F(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >)>::type>
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename network_boost::result_of<F(shared_future<R>)>::type>
  shared_future<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)  const
  {
    typedef typename network_boost::result_of<F(shared_future<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_!=0, future_uninitialized());

    network_boost::unique_lock<network_boost::mutex> lock(this->future_->mutex);
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((network_boost::detail::make_shared_future_async_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, network_boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((network_boost::detail::make_shared_future_deferred_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, network_boost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
    } else if (underlying_cast<int>(policy) & int(launch::executor)) {
      typedef executor Ex;
      Ex& ex = *(this->future_->get_executor());
      return BOOST_THREAD_MAKE_RV_REF((network_boost::detail::make_shared_future_executor_continuation_shared_state<Ex, shared_future<R>, future_type>(ex,
                    lock, *this, network_boost::forward<F>(func)
                )));
#endif
    } else if (underlying_cast<int>(policy) & int(launch::inherit)) {
...
namespace detail
{
  ...
  struct mfallbacker_to
  {
    ...
    : value_(network_boost::move(v))
    {}

    T operator()(BOOST_THREAD_FUTURE<T> fut) {
      return fut.get_or(network_boost::move(value_));
    }
  };
  template <typename T>
  ...
  BOOST_THREAD_FUTURE<R>::fallback_to(BOOST_THREAD_RV_REF(R2) v) {
    return then(detail::mfallbacker_to<R>(network_boost::move(v)));
  }
...
namespace detail
{
    ...
    void launch_continuation()
    {
      ...
      if (! unwrapped.valid() )
      {
        if (wrapped.has_exception()) {
          ...
          {
            lk.unlock();
            network_boost::unique_lock<network_boost::mutex> lk2(unwrapped.future_->mutex);
            unwrapped.future_->set_continuation_ptr(this->shared_from_this(), lk2);
          } else {
            this->mark_exceptional_finish_internal(network_boost::copy_exception(future_uninitialized()), lk);
          }
        }
      } else {
  ...
  make_future_unwrap_shared_state(network_boost::unique_lock<network_boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f) {
    shared_ptr<future_unwrap_shared_state<F, Rp> >
        h(new future_unwrap_shared_state<F, Rp>(network_boost::move(f)));
  ...
  BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >::unwrap()
  {
    ...
    return network_boost::detail::make_future_unwrap_shared_state<BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >, R2>(lock, network_boost::move(*this));
  }
#endif
...
namespace detail
{
  ...
  struct future_when_all_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<network_boost::detail::shared_state_base> that_) {
      ...
      try {
        network_boost::wait_for_all(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(network_boost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
  ...
  public:
    ...
    future_when_all_vector_shared_state(values_tag, BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
      vec_.push_back(network_boost::forward<T0>(f));
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(network_boost::forward<T>(futures)),'0'
          )..., '0'
  ...
  struct future_when_any_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<network_boost::detail::shared_state_base> that_)
    {
      ...
      try {
        network_boost::wait_for_any(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(network_boost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
  ...
  public:
    ...
    future_when_any_vector_shared_state(values_tag,
      ...
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(network_boost::forward<T>(futures))
          ,'0'
          )...,
          '0'
      }; //second part of magic unpacker
  ...
  struct wait_for_all_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      network_boost::wait_for_all(network_boost::forward<T>(v)...);
    }
  };
  ...
  struct wait_for_any_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      network_boost::wait_for_any(network_boost::forward<T>(v)...);
    }
  };
  ...
  struct future_when_all_tuple_shared_state: future_async_shared_state_base<Tuple>
  {
    ...
    static void run(shared_ptr<network_boost::detail::shared_state_base> that_) {
      ...
      try {
        // TODO make use of apply(that->tup_, network_boost::detail::wait_for_all_fctor());
        that->wait_for_all(Index());

        that->mark_finished_with_result(network_boost::move(that->tup_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
  ...
  public:
    ...
    future_when_all_tuple_shared_state(values_tag, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Fs) ... futures) :
      tup_(network_boost::csbl::make_tuple(network_boost::forward<F>(f), network_boost::forward<Fs>(futures)...))
    {
    }
  ...
  when_all(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
    ...
    shared_ptr<factory_type>
        h(new factory_type(detail::values_tag_value, network_boost::forward<T0>(f), network_boost::forward<T>(futures)...));
    h->init();
    return BOOST_THREAD_FUTURE<container_type>(h);
  }
#endif
