...
namespace network_boost
{
  namespace detail
  {
    ...
    {
      network_boost::unique_lock<MutexType1> l1(m1, network_boost::try_to_lock);
      if (!l1)
      {
        return 1;
      }
    ...
    {
      network_boost::unique_lock<MutexType1> l1(m1);
      if (!m2.try_lock())
      {
        return 1;
      }
  ...
  namespace detail
  {
...

    {
      ...
      Iterator const failed = network_boost::try_lock(++begin, end);
      if (failed == end)
      {
        guard.release();
      }
  ...
  namespace detail
  {
    ...
    void lock_impl(Iterator begin, Iterator end, is_mutex_type_wrapper<false> )
    {
      ...
      for (;;)
      {
        ...
        {
          begin_lock.lock();
          Iterator const failed_lock = network_boost::try_lock(next, end);
          if (failed_lock == end)
          {
            begin_lock.release();
            return;
        ...
        else
        {
          ...
          {
            Iterator const failed_lock = network_boost::try_lock(second, next);
            if (failed_lock == next)
            {
              begin_lock.release();
              guard.release();
