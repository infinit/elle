...
namespace network_boost
{
  struct exceptional_ptr {
    ...
    template <class E>
    explicit exceptional_ptr(BOOST_FWD_REF(E) ex) : ptr_(network_boost::copy_exception(network_boost::forward<E>(ex))) {}
  };

  template <class E>
  inline exceptional_ptr make_exceptional(BOOST_FWD_REF(E) ex) {
    return exceptional_ptr(network_boost::forward<E>(ex));
  }
