    ...
    class waiter_for_any_in_seq
    {
      ...
      struct all_futures_lock
      {
        ...
        all_futures_lock(std::vector<registered_waiter>& waiters) :
          count(waiters.size()), locks(new network_boost::unique_lock<network_boost::mutex>[count])
        {
          for (count_type_portable i = 0; i < count; ++i)
          {
            locks[i] = BOOST_THREAD_MAKE_RV_REF(network_boost::unique_lock<network_boost::mutex>(waiters[i].future_->mutex()));
          }
        }
