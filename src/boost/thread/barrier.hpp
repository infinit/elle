...
namespace network_boost
{
  namespace thread_detail
  {
    ...
    struct void_functor_barrier_reseter
    {
      ...
      : size_(size), fct_(network_boost::move(funct))
      {}
      template <typename F>
      ...
      //size_(BOOST_THREAD_RV(other).size_), fct_(network_boost::move(BOOST_THREAD_RV(other).fct_))
      {
      }
  ...
  class barrier
  {
    ...
    {
      if (count == 0) network_boost::throw_exception(
  ...
  public:
    ...
    bool wait()
    {
      network_boost::unique_lock < network_boost::mutex > lock(m_mutex);
