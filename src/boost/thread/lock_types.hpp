...
namespace network_boost
{
  ...
  public:
    ...
    {
      unique_lock temp(::network_boost::move(other));
      swap(temp);
      return *this;
    }
#endif
    ...
    void lock()
    {
      if (m == 0)
      {
        network_boost::throw_exception(
            network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock has no mutex"));
      }
      if (owns_lock())
      {
        network_boost::throw_exception(
            network_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur), "boost unique_lock owns already the mutex"));
      }
      m->lock();
      is_locked = true;
    }
    ...
    bool timed_lock(TimeDuration const& relative_time)
    {
      if(m==0)
      {
        network_boost::throw_exception(network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock has no mutex"));
      }
      if(owns_lock())
      {
        network_boost::throw_exception(network_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur), "boost unique_lock owns already the mutex"));
      }
      is_locked=m->timed_lock(relative_time);
      return is_locked;
    }

    bool timed_lock(::network_boost::system_time const& absolute_time)
    {
      if(m==0)
      {
        network_boost::throw_exception(network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock has no mutex"));
    ...
    bool timed_lock(::network_boost::xtime const& absolute_time)
    {
      if(m==0)
      {
        network_boost::throw_exception(network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock has no mutex"));
    ...
    void unlock()
    {
      ...
      {
        network_boost::throw_exception(
            network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock doesn't own the mutex"));
      }
      m->unlock();
      is_locked = false;
    }
  ...
  public:
    ...
    {
      shared_lock temp(::network_boost::move(other));
      swap(temp);
      return *this;
    }
#ifndef BOOST_THREAD_PROVIDES_EXPLICIT_LOCK_CONVERSION
    ...
    void lock()
    {
      if(m==0)
      {
        network_boost::throw_exception(network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost shared_lock has no mutex"));
      }
      if(owns_lock())
      {
        network_boost::throw_exception(network_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur), "boost shared_lock owns already the mutex"));
      }
      m->lock_shared();
      is_locked=true;
    }
...
#if defined BOOST_THREAD_USES_DATETIME
    bool timed_lock(network_boost::system_time const& target_time)
    {
      if(m==0)
      {
        network_boost::throw_exception(network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost shared_lock has no mutex"));
    ...
    void unlock()
    {
      ...
      if(!owns_lock())
      {
        network_boost::throw_exception(network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost shared_lock doesn't own the mutex"));
      }
      m->unlock_shared();
      is_locked=false;
    }
  ...
  public:
    ...
    {
      upgrade_lock temp(::network_boost::move(other));
      swap(temp);
      return *this;
    }
    ...
    void lock()
    {
      ...
      {
        network_boost::throw_exception(
            network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost shared_lock has no mutex"));
      }
      if (owns_lock())
      {
        network_boost::throw_exception(
            network_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur), "boost upgrade_lock owns already the mutex"));
      }
      m->lock_upgrade();
      is_locked = true;
    }
    ...
    void unlock()
    {
      ...
      {
        network_boost::throw_exception(
            network_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost upgrade_lock doesn't own the mutex"));
      }
      m->unlock_upgrade();
      is_locked = false;
    }
  ...
  public:
    ...
    explicit upgrade_to_unique_lock(upgrade_lock<Mutex>& m_) :
      source(&m_), exclusive(::network_boost::move(*source))
    {
    }
    ~upgrade_to_unique_lock()
    {
      if (source)
      {
        *source = BOOST_THREAD_MAKE_RV_REF(upgrade_lock<Mutex> (::network_boost::move(exclusive)));
      }
    }

    upgrade_to_unique_lock(BOOST_THREAD_RV_REF_BEG upgrade_to_unique_lock<Mutex> BOOST_THREAD_RV_REF_END other) BOOST_NOEXCEPT:
    source(BOOST_THREAD_RV(other).source),exclusive(::network_boost::move(BOOST_THREAD_RV(other).exclusive))
    {
...
private unique_lock<Mutex>
  ...
  public:
    ...
    base(::network_boost::move(other))
    {}

#elif defined BOOST_THREAD_USES_MOVE
    try_lock_wrapper(BOOST_THREAD_RV_REF(try_lock_wrapper) other):
    base(::network_boost::move(static_cast<base&>(other)))
    {}
