...
namespace network_boost
{
  ...
  {
#if defined BOOST_THREAD_USES_DATETIME
    inline struct timespec to_timespec(network_boost::system_time const& abs_time)
    {
      struct timespec timeout = { 0,0};
      network_boost::posix_time::time_duration const time_since_epoch=abs_time-network_boost::posix_time::from_time_t(0);
    ...
    inline timespec to_timespec(network_boost::intmax_t const& ns)
    {
      network_boost::intmax_t s = ns / 1000000000l;
      struct timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline network_boost::intmax_t to_nanoseconds_int_max(timespec const& ts)
    {
      return static_cast<network_boost::intmax_t>(ts.tv_sec) * 1000000000l + ts.tv_nsec;
    }
