...
template<class CharT>
inline bool
operator <( network_boost::unit_test::basic_cstring<CharT> const& x,
            network_boost::unit_test::basic_cstring<CharT> const& y )
{
    typedef typename network_boost::unit_test::basic_cstring<CharT>::traits_type traits_type;
    return x.size() != y.size()
            ? x.size() < y.size()
