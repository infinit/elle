...
private:
    struct nil_t {};
    template<typename U> static auto  test( U* ) -> decltype(network_boost::declval<U>().size());
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(network_boost::declval<U>().begin());
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(network_boost::declval<U>().end());
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
