...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(network_boost::declval<Lhs>() oper network_boost::declval<Rhs>() ) optype; \
    typedef typename network_boost::remove_reference<optype>::type              \
/**/
#else
...
#define ADD_OP_SUPPORT( oper, name, _ )                         \
    template<typename T>                                        \
    binary_expr<ExprType,typename network_boost::decay<T const>::type,  \
        op::name<ValType,typename network_boost::decay<T const>::type> >\
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename network_boost::decay<T const>::type Rhs;       \
