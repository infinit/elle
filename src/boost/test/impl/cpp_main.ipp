...
prg_exec_monitor_main( int (*cpp_main)( int argc, char* argv[] ), int argc, char* argv[] )
{
    ...
    BOOST_TEST_I_TRY {
        network_boost::unit_test::const_string p( std::getenv( "BOOST_TEST_CATCH_SYSTEM_ERRORS" ) );
        ::network_boost::execution_monitor ex_mon;
        ...
        if( result == 0 )
            result = ::network_boost::exit_success;
        else if( result != ::network_boost::exit_success ) {
            std::cout << "\n**** error return code: " << result << std::endl;
            result = ::network_boost::exit_failure;
        }
    }
    BOOST_TEST_I_CATCH( ::network_boost::execution_exception, exex ) {
        std::cout << "\n**** exception(" << exex.code() << "): " << exex.what() << std::endl;
        result = ::network_boost::exit_exception_failure;
    }
    BOOST_TEST_I_CATCH( ::network_boost::system_error, ex ) {
    ...
    if( result != ::network_boost::exit_success ) {
        ...
        ::network_boost::unit_test::const_string p( std::getenv( "BOOST_PRG_MON_CONFIRM" ) );
        if( p != "no" ) {
...
{
    return ::network_boost::prg_exec_monitor_main( &cpp_main, argc, argv );
}
