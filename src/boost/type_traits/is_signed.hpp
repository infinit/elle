...
{
   typedef typename remove_cv<T>::type no_cv_t;
   BOOST_STATIC_CONSTANT(bool, value = (!(::network_boost::detail::is_signed_values<T>::minus_one  > network_boost::detail::is_signed_values<T>::zero)));
};
...
{
   typedef ::network_boost::detail::is_signed_select_helper< ::network_boost::is_integral<T>::value || ::network_boost::is_enum<T>::value> selector;
...
template <class T> struct is_signed : public integral_constant<bool, network_boost::detail::is_signed_impl<T>::value> {};
...
template <> struct is_signed< ::network_boost::long_long_type> : public true_type{};
template <> struct is_signed<const ::network_boost::long_long_type> : public true_type{};
template <> struct is_signed<volatile ::network_boost::long_long_type> : public true_type{};
template <> struct is_signed<const volatile ::network_boost::long_long_type> : public true_type{};

template <> struct is_signed< ::network_boost::ulong_long_type> : public false_type{};
template <> struct is_signed<const ::network_boost::ulong_long_type> : public false_type{};
template <> struct is_signed<volatile ::network_boost::ulong_long_type> : public false_type{};
template <> struct is_signed<const volatile ::network_boost::ulong_long_type> : public false_type{};
#endif
#if defined(CHAR_MIN) 
#if CHAR_MIN != 0
