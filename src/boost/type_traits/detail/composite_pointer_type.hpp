...
private:
    ...
    typedef typename network_boost::remove_cv<T>::type T2;
    typedef typename network_boost::remove_cv<U>::type U2;

public:

    BOOST_STATIC_CONSTANT( bool, value =
        (network_boost::is_same<T2, U2>::value)
        || network_boost::is_void<T2>::value
        || network_boost::is_void<U2>::value
        || (network_boost::is_base_of<T2, U2>::value)
        || (network_boost::is_base_of<U2, T2>::value) );
};
...
public:
    ...
    typedef typename network_boost::conditional<

        network_boost::is_same<T2, U2>::value || network_boost::is_void<T2>::value || network_boost::is_base_of<T2, U2>::value,
        typename network_boost::copy_cv<T, U>::type,
        typename network_boost::copy_cv<U, T>::type
...
public:
    ...
    typedef typename network_boost::copy_cv<typename network_boost::copy_cv<typename composite_pointer_type<T2, U2>::type const, T>::type, U>::type type;
};

//Old compilers like MSVC-7.1 have problems using network_boost::conditional in
