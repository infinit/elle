...
template <class T> struct is_copy_constructible : public network_boost::is_constructible<T, const T&>{};
...
// network_boost::non_copyable as a base class as a special case:
//
...
namespace network_boost {
   ...
   namespace detail
   {
      ...
      template <class T, bool b> struct is_copy_constructible_imp : public network_boost::is_constructible<T, const T&>{};
      template <class T> struct is_copy_constructible_imp<T, true> : public false_type{};

   }

   template <class T> struct is_copy_constructible : public detail::is_copy_constructible_imp<T, is_base_and_derived<network_boost::noncopyable, T>::value>{};
...
namespace network_boost {
   ...
   namespace detail{
      ...
      struct is_copy_constructible_impl2 {
         ...
         template <class T1>
         static network_boost::type_traits::yes_type test(const T1&, network_boost::mpl::int_<sizeof(T1(network_boost::declval<const T1&>()))>* = 0);
#else
         template <class T1>
         static network_boost::type_traits::yes_type test(const T1&, decltype(T1(network_boost::declval<const T1&>()))* = 0);
#endif

         static network_boost::type_traits::no_type test(...);
#else
         template <class T1>
         static network_boost::type_traits::no_type test(const T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
         static network_boost::type_traits::yes_type test(...);
#endif
         ...
         //      struct T: private network_boost::noncopyable {
         //          ...
         //      private:
         //          T(const T &);
         //          ...
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(
            network_boost::declval<BOOST_DEDUCED_TYPENAME network_boost::add_reference<T const>::type>()
            )) == sizeof(network_boost::type_traits::yes_type)
            &&
            !network_boost::is_rvalue_reference<T>::value
            && !network_boost::is_array<T>::value
            ));
      };
      ...
      struct is_copy_constructible_impl {
         ...
         BOOST_STATIC_CONSTANT(bool, value = (
            network_boost::detail::is_copy_constructible_impl2<
            network_boost::is_base_and_derived<network_boost::noncopyable, T>::value,
            T
            >::value
            ));
      };
   ...
   template <class T> struct is_copy_constructible : public integral_constant<bool, ::network_boost::detail::is_copy_constructible_impl<T>::value>{};
   template <> struct is_copy_constructible<void> : public false_type{};
