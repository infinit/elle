...
namespace network_boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename = decltype(network_boost::declval<T&>().~T())>
   ...
   template <class T> struct is_destructible : public integral_constant<bool, sizeof(detail::is_destructible_imp::test<T>(0)) == sizeof(network_boost::type_traits::yes_type)>{};
