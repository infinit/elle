...
namespace network_boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename ...TheArgs, typename = decltype(T(network_boost::declval<TheArgs>()...))>
         static network_boost::type_traits::yes_type test(int);
         template<typename, typename...>
         static network_boost::type_traits::no_type test(...);

         template<typename T, typename Arg, typename = decltype(::new T(network_boost::declval<Arg>()))>
         static network_boost::type_traits::yes_type test1(int);
         template<typename, typename>
         static network_boost::type_traits::no_type test1(...);

         template <typename T>
         static network_boost::type_traits::yes_type ref_test(T);
         template <typename T>
         static network_boost::type_traits::no_type ref_test(...);
      };

   }

   template <class T, class ...Args> struct is_constructible : public integral_constant<bool, sizeof(detail::is_constructible_imp::test<T, Args...>(0)) == sizeof(network_boost::type_traits::yes_type)>{};
   template <class T, class Arg> struct is_constructible<T, Arg> : public integral_constant<bool, is_destructible<T>::value && sizeof(detail::is_constructible_imp::test1<T, Arg>(0)) == sizeof(network_boost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&>(network_boost::declval<Arg>())) == sizeof(network_boost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&&>(network_boost::declval<Arg>())) == sizeof(network_boost::type_traits::yes_type)>{};
