...
namespace network_boost {
   ...
   namespace detail
   {
      ...
      template <class T>          struct has_nothrow_assign_imp<T, false, true>{ static const bool value = noexcept(network_boost::declval<typename add_reference<T>::type>() = network_boost::declval<typename add_reference<T const>::type>()); };
   ...
   struct has_nothrow_assign : public integral_constant < bool,
      ...
      >::value
#else
      ::network_boost::has_trivial_assign<T>::value
#endif
#else
