...
namespace network_boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename U, typename = decltype(network_boost::declval<T>() = network_boost::declval<U>())>
   ...
   template <class T, class U> struct is_assignable : public integral_constant<bool, sizeof(detail::is_assignable_imp::test<T, U>(0)) == sizeof(network_boost::type_traits::yes_type)>{};
