...
#define BOOST_TEST_BUILD_ASSERTION( P )                         \
    ::network_boost::test_tools::tt_detail::expression_holder const& E= \
    ::network_boost::test_tools::tt_detail::hold_expression(            \
        ::network_boost::test_tools::assertion::seed() ->* P )          \
/**/
#else
#define BOOST_TEST_BUILD_ASSERTION( P )                         \
    auto const& E = ::network_boost::test_tools::assertion::seed()->*P  \
/**/
#endif
...
do {                                                            \
    ::network_boost::test_tools::assertion_result res = (P);            \
    report_assertion(                                           \
        res,                                                    \
        BOOST_TEST_LAZY_MSG( M ),                               \
        BOOST_TEST_L(__FILE__),                                 \
        static_cast<std::size_t>(__LINE__),                     \
        ::network_boost::test_tools::tt_detail::level,                  \
        ::network_boost::test_tools::tt_detail::CHECK_MSG,              \
        0 );                                                    \
...
do {                                                            \
    ...
    report_assertion(                                           \
      ...
      static_cast<std::size_t>(__LINE__),                       \
      ::network_boost::test_tools::tt_detail::level,                    \
      ::network_boost::test_tools::tt_detail::CHECK_BUILT_ASSERTION,    \
      0 );                                                      \
} while( ::network_boost::test_tools::tt_detail::dummy_cond() )         \
/**/
...
do {                                                            \
    ...
    ::network_boost::test_tools::tt_detail::                            \
    report_assertion(                                           \
      ::network_boost::test_tools::tt_detail::assertion_evaluate(E)     \
          << arg,                                               \
      ::network_boost::test_tools::tt_detail::assertion_text(           \
          BOOST_TEST_LAZY_MSG( BOOST_TEST_STRINGIZE(P) ),       \
          BOOST_TEST_LAZY_MSG( arg ) ),                         \
      BOOST_TEST_L(__FILE__),                                   \
      static_cast<std::size_t>(__LINE__),                       \
      ::network_boost::test_tools::tt_detail::level,                    \
      ::network_boost::test_tools::tt_detail::assertion_type()          \
          << arg,                                               \
      0 );                                                      \
} while( ::network_boost::test_tools::tt_detail::dummy_cond() )         \
/**/
...
#define BOOST_TEST_TOOL_UNIV( level, P )                                    \
do {                                                                        \
    if( ::network_boost::debug::under_debugger() )                                  \
        BOOST_TEST_TOOL_DIRECT_IMPL( P, level, BOOST_TEST_STRINGIZE( P ) ); \
    else                                                                    \
...
#define BOOST_TEST_IS_DEFINED( symb ) ::network_boost::test_tools::tt_detail::is_defined_impl( symb, BOOST_STRINGIZE(= symb) )
...
#define BOOST_WARN_CLOSE( L, R, T )         BOOST_TEST_WARN( L == R, T % ::network_boost::test_tools::tolerance() )
#define BOOST_CHECK_CLOSE( L, R, T )        BOOST_TEST_CHECK( L == R, T % ::network_boost::test_tools::tolerance() )
#define BOOST_REQUIRE_CLOSE( L, R, T )      BOOST_TEST_REQUIRE( L == R, T % ::network_boost::test_tools::tolerance() )

#define BOOST_WARN_CLOSE_FRACTION(L, R, T)  BOOST_TEST_WARN( L == R, ::network_boost::test_tools::tolerance( T ) )
#define BOOST_CHECK_CLOSE_FRACTION(L, R, T) BOOST_TEST_CHECK( L == R, ::network_boost::test_tools::tolerance( T ) )
#define BOOST_REQUIRE_CLOSE_FRACTION(L,R,T) BOOST_TEST_REQUIRE( L == R, ::network_boost::test_tools::tolerance( T ) )

#define BOOST_WARN_SMALL( FPV, T )          BOOST_TEST_WARN( FPV == 0., ::network_boost::test_tools::tolerance( T ) )
#define BOOST_CHECK_SMALL( FPV, T )         BOOST_TEST_CHECK( FPV == 0., ::network_boost::test_tools::tolerance( T ) )
#define BOOST_REQUIRE_SMALL( FPV, T )       BOOST_TEST_REQUIRE( FPV == 0., ::network_boost::test_tools::tolerance( T ) )

//____________________________________________________________________________//

#define BOOST_WARN_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_WARN( ::network_boost::test_tools::tt_detail::make_it_pair(L_begin, L_end) ==\
                     ::network_boost::test_tools::tt_detail::make_it_pair(R_begin, R_end),  \
                     ::network_boost::test_tools::per_element() )                           \
/**/

#define BOOST_CHECK_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_CHECK( ::network_boost::test_tools::tt_detail::make_it_pair(L_begin, L_end) ==\
                      ::network_boost::test_tools::tt_detail::make_it_pair(R_begin, R_end),  \
                      ::network_boost::test_tools::per_element() )                           \
/**/

#define BOOST_REQUIRE_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_REQUIRE( ::network_boost::test_tools::tt_detail::make_it_pair(L_begin, L_end) ==\
                        ::network_boost::test_tools::tt_detail::make_it_pair(R_begin, R_end),  \
                        ::network_boost::test_tools::per_element() )                           \
/**/
...
#define BOOST_WARN_BITWISE_EQUAL( L, R )    BOOST_TEST_WARN( L == R, ::network_boost::test_tools::bitwise() )
#define BOOST_CHECK_BITWISE_EQUAL( L, R )   BOOST_TEST_CHECK( L == R, ::network_boost::test_tools::bitwise() )
#define BOOST_REQUIRE_BITWISE_EQUAL( L, R ) BOOST_TEST_REQUIRE( L == R, ::network_boost::test_tools::bitwise() )
