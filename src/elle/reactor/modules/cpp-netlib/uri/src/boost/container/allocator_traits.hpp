...
#define BOOST_INTRUSIVE_HAS_MEMBER_FUNCTION_CALLABLE_WITH_NS_BEG namespace network_boost { namespace container { namespace container_detail {
...
struct allocator_traits
{
   ...
   typedef typename allocator_type::value_type value_type;
   ...
   #else
      //pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_DEFAULT(network_boost::container::container_detail::, Allocator,
         pointer, value_type*)
            pointer;
      //const_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(network_boost::container::container_detail::, Allocator,
         const_pointer, typename network_boost::intrusive::pointer_traits<pointer>::template
      ...
      //void_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(network_boost::container::container_detail::, Allocator,
         void_pointer, typename network_boost::intrusive::pointer_traits<pointer>::template
            rebind_pointer<void>)
               void_pointer;
      //const_void_pointer
      typedef BOOST_INTRUSIVE_OBTAIN_TYPE_WITH_EVAL_DEFAULT(network_boost::container::container_detail::, Allocator,
         const_void_pointer, typename network_boost::intrusive::pointer_traits<pointer>::template
      ...
      //rebind_alloc & rebind_traits
         ...
         //C++11
         template <typename T> using rebind_alloc  = typename network_boost::intrusive::pointer_rebind<Allocator, T>::type;
         template <typename T> using rebind_traits = allocator_traits< rebind_alloc<T> >;
      #else    // #if !defined(BOOST_NO_CXX11_TEMPLATE_ALIASES)
         //Some workaround for C++03 or C++11 compilers with no template aliases
         template <typename T>
         struct rebind_alloc : network_boost::intrusive::pointer_rebind<Allocator,T>::type
         {
            typedef typename network_boost::intrusive::pointer_rebind<Allocator,T>::type Base;
            #if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
               template <typename... Args>
               rebind_alloc(BOOST_FWD_REF(Args)... args) : Base(network_boost::forward<Args>(args)...) {}
         ...
         struct rebind_traits
            : allocator_traits<typename network_boost::intrusive::pointer_rebind<Allocator, T>::type>
         {};
      ...
      template <class T>
      struct portable_rebind_alloc
      {  typedef typename network_boost::intrusive::pointer_rebind<Allocator, T>::type type;  };
   ...
   {
      const bool value = network_boost::container::container_detail::
   ...
   select_on_container_copy_construction(const Allocator &a)
   {
      ...
      {
         static const bool value = ::network_boost::move_detail::and_
            < container_detail::is_not_std_allocator<Allocator>
            , network_boost::container::container_detail::has_member_function_callable_with_construct
                  < Allocator, T*, Args... >
            >::value;
         container_detail::bool_<value> flag;
         allocator_traits::priv_construct(flag, a, p, ::network_boost::forward<Args>(args)...);
      }
   #endif
   ...
   {  return a;  }
      ...
      {  a.construct( p, ::network_boost::forward<Args>(args)...);  }

      template<class T, class ...Args>
      static void priv_construct(container_detail::false_type, Allocator &, T *p, BOOST_FWD_REF(Args) ...args)
      {  ::new((void*)p, boost_container_new_t()) T(::network_boost::forward<Args>(args)...); }
   #else // #if !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
      public:
   ...
   template<class T>
   static void priv_construct(container_detail::false_type, Allocator &, T *p, const ::network_boost::container::default_init_t&)
