...
struct sub_match : public std::pair<BidiIterator, BidiIterator>
{
   ...
   {
      difference_type n = matched ? ::network_boost::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)this->first, (BidiIterator)this->second) : 0;
      return n;
   }
   std::basic_string<value_type> str()const
   {
      ...
      if(matched)
      {
         std::size_t len = ::network_boost::BOOST_REGEX_DETAIL_NS::distance((BidiIterator)this->first, (BidiIterator)this->second);
         result.reserve(len);
...
private:
   mutable network_boost::scoped_ptr<capture_sequence_type> m_captures;
public:
