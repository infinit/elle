...
#include <boost/utility/enable_if.hpp> // for network_boost::disable_if

namespace network_boost { namespace algorithm {

/// \fn clamp ( T const& val, 
///               typename network_boost::mpl::identity<T>::type const & lo, 
///               typename network_boost::mpl::identity<T>::type const & hi, Pred p )
...
/// \fn clamp ( T const& val, 
///               typename network_boost::mpl::identity<T>::type const & lo, 
///               typename network_boost::mpl::identity<T>::type const & hi )
/// \return the value "val" brought into the range [ lo, hi ].
...
///       typename std::iterator_traits<typename network_boost::range_iterator<const Range>::type>::value_type const & lo,
///       typename std::iterator_traits<typename network_boost::range_iterator<const Range>::type>::value_type const & hi )
/// \return clamp the sequence of values [first, last) into [ lo, hi ]
/// 
...
///
  ...
  typename network_boost::disable_if_c<network_boost::is_same<Range, OutputIterator>::value, OutputIterator>::type
  clamp_range ( const Range &r, OutputIterator out,
    typename std::iterator_traits<typename network_boost::range_iterator<const Range>::type>::value_type const & lo, 
    typename std::iterator_traits<typename network_boost::range_iterator<const Range>::type>::value_type const & hi )
  {
    return clamp_range ( network_boost::begin ( r ), network_boost::end ( r ), out, lo, hi );
  } 
...
//  In that case, the user will get the two iterator version.
  ...
  clamp_range ( const Range &r, OutputIterator out,
    ...
    Pred p )
  {
    return clamp_range ( network_boost::begin ( r ), network_boost::end ( r ), out, lo, hi, p );
  } 
