...
network_boost
    ...
    exception_detail
        {
        std::string diagnostic_information_impl( network_boost::exception const *, std::exception const *, bool, bool );
        }
    ...
    current_exception_diagnostic_information( bool verbose=true)
        {
        network_boost::exception const * be=current_exception_cast<network_boost::exception const>();
...
network_boost
    ...
    exception_detail
        ...
        inline
        std::string
        diagnostic_information_impl( network_boost::exception const * be, std::exception const * se, bool with_what, bool verbose )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef BOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<network_boost::exception const *>(se);
            if( !se )
    ...
    diagnostic_information_what( exception const & e, bool verbose=true ) throw()
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce network_boost::diagnostic_information_what()";
#ifndef BOOST_NO_EXCEPTIONS
            }
        catch(
        ... )
