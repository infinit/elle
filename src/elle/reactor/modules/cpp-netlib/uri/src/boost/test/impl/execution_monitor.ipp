...
extract( network_boost::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = network_boost::get_error_info<ErrorInfo>( *ex );
...
static void
report_error( execution_exception::error_code ec, network_boost::exception const* be, char const* format, va_list* args )
{
...
static void
report_error( execution_exception::error_code ec, network_boost::exception const* be, char const* format, ... )
{
    va_list args;
...
// ************************************************************************** //
// **************    network_boost::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************         network_boost::detail::signal_action         ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************        network_boost::detail::signal_handler         ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( network_boost::function<int ()> const& F )
{
...
int
execution_monitor::execute( network_boost::function<int ()> const& F )
{
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<network_boost::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
#undef CATCH_AND_REPORT_STD_EXCEPTION
    ...
    catch( network_boost::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
#ifdef BOOST_NO_TYPEID
                              "unknown network_boost::exception" ); }
#else
                              typeid(ex).name()          ); }
#endif
...
struct forward {
    explicit    forward( network_boost::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    network_boost::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( network_boost::function<void ()> const& F )
{
...
{
    network_boost::ignore_unused(mask);
