...
namespace network_boost { namespace algorithm { namespace detail {
...
//  General case for data searching other than bytes; use a map
    ...
    private:
#ifdef BOOST_NO_CXX11_HDR_UNORDERED_MAP
        typedef network_boost::unordered_map<key_type, value_type> skip_map;
#else
...
//  Special case small numeric values; use an array
    ...
    private:
        typedef typename network_boost::make_unsigned<key_type>::type unsigned_key_type;
        typedef network_boost::array<value_type, 1U << (CHAR_BIT * sizeof(key_type))> skip_map;
        skip_map skip_;
        const value_type k_default_value;
    public:
    ...
    struct BM_traits {
        ...
        typedef network_boost::algorithm::detail::skip_table<key_type, value_type, 
                network_boost::is_integral<key_type>::value && (sizeof(key_type)==1)> skip_table_t;
        };
