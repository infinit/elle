...
ELLE_LOG_COMPONENT("reactor.network.utp.socket");
...
namespace reactor
{
  namespace network
  {
    ...
    UTPSocket::~UTPSocket()
    {
      ELLE_DEBUG("~UTPSocket %s", (void*) this);
      this->on_close();
      reactor::wait(_pending_operations);
    }

    void
    UTPSocket::connect(std::string const& id,
    ...
    UTPSocket::on_close()
    {
      ELLE_DEBUG("closing socket %s", *this);
      if (this->_closing)
        return;
      this->_closing = true;
      auto it = std::find(
