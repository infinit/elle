...
namespace reactor
{
  ...
  /// terminated upon destruction.  As a Barrier, a Scope is open if and only if
  /// all currently managed threads are done, enabling to easily join the group
  /// of managed threads.  If an exception escapes from a thread, all other
  /// threads are killed and waiting the Scope will re-throw the exception.
  class Scope:
