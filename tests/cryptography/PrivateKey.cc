...
test_generate_rsa(elle::Natural32 const length)
{
  // By implementation.
  ...
  infinit::cryptography::PrivateKey k(std::move(inteface));
...
{
  // RSA.
  infinit::cryptography::PrivateKey k =
...
test_compare_rsa()
{
  infinit::cryptography::PrivateKey k1 =
    test_generate_rsa(1024);

  infinit::cryptography::PrivateKey k2 =
    test_generate_rsa(1024);
...
test_serialize_rsa()
{
  // Serialize/deserialize.
  {
    ...
    infinit::cryptography::PrivateKey k2; // XXX[load construct]
    ELLE_ASSERT(false);
...
test()
{
  boost::unit_test::test_suite* suite = BOOST_TEST_SUITE("PrivateKey");
