...
TEST(uri_normalization_test, normalize_case) {
  network::uri instance("HTTP://www.example.com/");
  ASSERT_EQ("http://www.example.com/",
	    instance.normalize(network::uri_comparison_level::case_normalization));
}
...
TEST(uri_normalization_test, normalize_percent_encoding) {
  network::uri instance("http://www%2Eexample%2Ecom/%7E%46%4F%4F%42%41%52%5F%36%39/");
  ASSERT_EQ("http://www.example.com/~foobar_69/",
	    instance.normalize(network::uri_comparison_level::percent_encoding_normalization));
}
...
TEST(uri_normalization_test, path_segmented_add_trailing_slash) {
  network::uri instance("http://www.example.com");
  ASSERT_EQ("http://www.example.com/",
	    instance.normalize(network::uri_comparison_level::path_segment_normalization));
}
